...
<details>
  <summary>Details</summary>
Motivation: 传统脑机接口（如SSVEP-BCI和MI-BCI）依赖非自然范式，应用场景有限。BMHI通过模仿自然神经运动通路，旨在实现更自然的交互模式。

Method: 提出BMHI（BM + MH），通过离线、对比和在线实验验证其有效性。对比传统单步策略（脑-手接口），BMHI采用分阶段解码策略。

Result: BMHI预测精度0.79，训练时间减少18倍，解码精度提高。在线实验中，虚拟手和机械手成功完成日常任务。

Conclusion: BMHI的两步策略模仿自然神经通路，提升训练效率和预测精度，推动脑机接口技术向更自然的交互模式发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型在AI研究中的兴起，构建大规模大脑基础模型以推动神经科学发展成为研究热点。现有模型多基于时间序列或ROI特征，本文提出图预训练范式以提升泛化能力。

Method: 提出BrainGFM框架，结合图对比学习和图掩码自编码器，预训练于多种脑图谱数据。引入图和语言提示，支持灵活下游任务迁移，并通过元学习优化提示。

Result: 在27个神经影像数据集上预训练，涵盖25种疾病、8种分区和25,000名受试者，模型在少样本和零样本条件下表现优异。

Conclusion: BrainGFM为大脑图基础模型提供了统一框架，支持跨领域泛化和多任务适应，代码已开源。
...
TL;DR: GLM-Transformer模型结合深度隐变量模型和点过程广义线性模型，用于识别神经群体间的功能耦合，同时处理非稳态背景动态。


<details>
  <summary>Details</summary>
Motivation: 大规模神经记录技术提供了研究神经回路信号传输的机会，但神经响应的试次间变异性（如行为或内部状态变化）可能干扰功能耦合的识别。

Method: 开发了GLM-Transformer模型，结合Transformer变分自编码器（捕捉非稳态动态）和GLM（估计群体间交互），并引入低秩结构提高可扩展性。

Result: 在合成数据和模拟中，模型能准确恢复已知耦合结构并抵抗背景波动；应用于Allen视觉编码数据集时，识别出与视觉层级一致的反馈通路。

Conclusion: GLM-Transformer为神经群体交互的识别提供了改进方法，结合了深度学习的优势与可解释性。
...
<details>
  <summary>Details</summary>
Motivation: 探索大脑如何动态组织异质区域形成任务相关的通路。

Method: 扩展了异质混合专家架构，引入三种生物相关归纳偏置：路由成本、任务性能相关的成本缩放因子和随机专家丢弃。

Result: 人工通路模型与大脑在学习任务中的表现一致，验证了归纳偏置的有效性。

Conclusion: 提出的框架为研究大脑通路形成提供了新视角，并可能提升混合专家架构的适应性。
...
<details>
  <summary>Details</summary>
Motivation: 听觉大脑的建模因复杂性高而难以手工完成，且缺乏直接训练DNN模型的数据集。现有模型假设所有大脑的听觉处理相同，无法捕捉听力损失的多样性。

Method: 使用变分条件模型，从健康及噪声暴露动物的听觉中脑神经活动中学习听力损失空间，参数化仅需6个自由参数。

Result: 模型准确预测了正常听力动物62%和听力受损动物68%的可解释神经反应方差，接近最优动物特定模型。通过贝叶斯优化，仅需15-30次迭代即可拟合新动物的参数。

Conclusion: 该模型为未来开发参数化听力损失补偿模型奠定了基础，可通过快速拟合新用户参数直接恢复听力受损大脑的正常神经编码。
...
### [16] [Quantifying task-relevant representational similarity using decision variable correlation](https://arxiv.org/abs/2506.02164)
...
### [17] [Explicitly Modeling Subcortical Vision with a Neuro-Inspired Front-End Improves CNN Robustness](https://arxiv.org/abs/2506.03089)
...
### [18] [Quantifying task-relevant representational similarity using decision variable correlation](https://arxiv.org/abs/2506.02164)
*Yu,Qian,Wilson S. Geisler,Xue-Xin Wei*
...
### [19] [Explicitly Modeling Subcortical Vision with a Neuro-Inspired Front-End Improves CNN Robustness](https://arxiv.org/abs/2506.03089)
*Lucas Piper,Arlindo L. Oliveira,Tiago Marques*
...
### [20] [Quantifying task-relevant representational similarity using decision variable correlation](https://arxiv.org/abs/2506.02164)
*Yu,Qian,Wilson S. Geisler,Xue-Xin Wei*
...
Method: 使用决策变量相关（DVC）方法量化模型和猴子在分类任务中的决策策略相似性，并评估其在猴子V4/IT记录和模型上的表现。

Result: 模型与模型之间的相似性与猴子之间的相似性相当，但模型与猴子的相似性较低且随模型性能提升而降低；对抗训练和更大数据集预训练未能提升模型与猴子的相似性。
...
### [21] [Explicitly Modeling Subcortical Vision with a Neuro-Inspired Front-End Improves CNN Robustness](https://arxiv.org/abs/2506.03089)
*Lucas Piper,Arlindo L. Oliveira,Tiago Marques*
...
<details>
  <summary>Details</summary>
Motivation: 解决CNN在视觉扰动和域外图像中的脆弱性问题，通过模拟生物视觉系统提升模型鲁棒性。

Method: 提出EVNets，结合VOneBlock和SubcorticalBlock，后者基于神经科学计算模型设计。

Result: EVNets在V1对齐、形状偏好和鲁棒性基准测试中表现优于基础CNN，提升8.5%。结合数据增强后性能进一步提升7.3%。
...
### [22] [Memorization to Generalization: Emergence of Diffusion Models from Associative Memory](https://arxiv.org/abs/2505.21777)
...
### [23] [Memorization to Generalization: Emergence of Diffusion Models from Associative Memory](https://arxiv.org/abs/2505.21777)
*Bao Pham,Gabriel Raya,Matteo Negri,Mohammed J. Zaki,Luca Ambrogioni,Dmitry Krotov*
...
### [24] [Memorization to Generalization: Emergence of Diffusion Models from Associative Memory](https://arxiv.org/abs/2505.21777)
*Bao Pham,Gabriel Raya,Matteo Negri,Mohammed J. Zaki,Luca Ambrogioni,Dmitry Krotov*
...
<details>
  <summary>Details</summary>
Motivation: 研究扩散模型在联想记忆框架下的行为，探索其记忆编码与检索机制，特别是虚假状态的出现。

Method: 将扩散模型的训练和生成阶段分别类比为记忆编码和检索，分析其在不同数据量下的行为。

Result: 在小数据量下，扩散模型表现出强记忆性；大数据量下出现新的吸引子状态，虚假状态在过渡边界出现。

Conclusion: 扩散模型的记忆-泛化现象可通过联想记忆理论解释，虚假状态的存在得到理论和实证验证。
