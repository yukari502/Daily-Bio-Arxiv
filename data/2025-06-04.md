...
<details>
  <summary>Details</summary>
Motivation: 传统脑机接口（如SSVEP-BCI和MI-BCI）依赖非自然范式，应用场景有限。BMHI旨在通过模拟自然神经运动通路，提升交互的自然性和效率。

Method: 提出BMHI（BM + MH），通过离线、对比（BMHI与传统脑-手接口）和在线实验（控制虚拟/机械手完成任务）验证其有效性。

Result: BMHI预测准确率为0.79，训练时间减少约18倍，解码精度提高。在线实验中成功完成日常任务。

Conclusion: BMHI的两步解码策略模拟自然神经通路，提升训练效率和预测精度，推动脑机接口技术向更自然的交互模式发展。
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型（LLM）的发展，构建脑科学领域的大规模基础模型成为研究热点。现有模型多基于时间序列或ROI特征，而本文提出图预训练范式以提升模型的泛化能力。

Method: BrainGFM采用图对比学习和图掩码自编码器进行预训练，整合多样脑图谱数据，并通过图提示和语言提示支持下游任务迁移。此外，利用元学习优化图提示，实现零样本和少样本学习。

Result: 模型在27个神经影像数据集上预训练，涵盖25种神经和精神疾病、8种常用脑图谱分区，包含25,000名受试者和60,000次fMRI扫描，性能显著。

Conclusion: BrainGFM为脑科学提供了一种灵活且高效的图预训练框架，支持跨异构数据和任务的泛化应用。
...
Method: 开发了GLM-Transformer模型，结合Transformer变分自编码器（捕捉非平稳神经元动态）和GLM（估计群体间相互作用），并采用低秩结构提高可扩展性。

Result: 在合成数据和模拟中，模型能恢复已知耦合结构并抵抗背景波动；应用于Allen Institute视觉编码数据集时，识别出与视觉层级一致的反馈通路。
...
<details>
  <summary>Details</summary>
Motivation: 探讨大脑区域如何动态组织成任务相关的通路，尤其是在学习和任务解决过程中。

Method: 使用扩展的混合专家架构（Mixture-of-Pathways模型），引入三种归纳偏置：路由成本、任务性能相关的成本调节和随机专家丢弃。

Result: 人工模型中的通路形成与大脑在学习和任务解决中的通路使用方式一致。

Conclusion: 提出了一种新框架，通过归纳偏置研究大脑任务通路的形成，可能使混合专家架构更具适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型假设所有大脑的听觉处理相同，无法捕捉听力损失的广泛变化效应。因此，需要一种能够直接编码听力损失空间的方法。

Method: 采用变分条件模型，从健康动物和噪声暴露动物的听觉中脑神经活动记录中学习，参数化听力损失。

Result: 模型准确预测了62%的正常听力动物和68%的听力受损动物的可解释神经响应方差，并通过贝叶斯优化快速拟合新动物数据。

Conclusion: 该模型为未来开发参数化听力损失补偿模型奠定了基础，可通过快速拟合新用户数据恢复听力受损大脑的正常神经编码。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是解决先前关于大脑和深度神经网络在图像分类任务中表征相似性的争议，提出一种更直接比较决策策略的方法。

Method: 采用决策变量相关性（DVC）方法，量化分类任务中解码决策的相关性，评估模型与猴子V4/IT记录的相似性。

Result: 发现模型之间相似性与猴子之间相似性相当，但模型与猴子相似性较低且随模型性能提升而下降；对抗训练和更大数据集预训练未能改善模型与猴子的相似性。

Conclusion: 结果表明，猴子V4/IT与图像分类模型在任务相关表征上存在根本差异。
...
Method: 提出EVNets，结合VOneBlock和SubcorticalBlock，优化与生物视觉系统的对齐性。

Result: EVNets在V1对齐性、形状偏置和鲁棒性测试中表现优于基准CNN，提升8.5%。与数据增强结合后，性能进一步提升7.3%。
...
<details>
  <summary>Details</summary>
Motivation: 研究扩散模型在记忆编码和检索过程中的行为，特别是其在小数据和大数据情况下的表现，以理解其记忆与泛化机制。

Method: 将扩散模型的训练阶段视为记忆编码，生成阶段视为记忆检索，并分析其在不同数据量下的行为。

Result: 在小数据情况下，扩散模型表现出强记忆性；在大数据情况下，出现新的吸引状态，并验证了虚假状态的存在。

Conclusion: 通过联想记忆的视角，为扩散模型的记忆与泛化现象提供了新的理解，并预测和验证了虚假状态的存在。
