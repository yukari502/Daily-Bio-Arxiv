<div id=toc></div>

# Table of Contents

- [q-bio.GN](#q-bio.GN) [Total: 3]
- [q-bio.QM](#q-bio.QM) [Total: 6]
- [eess.IV](#eess.IV) [Total: 3]
- [cs.LG](#cs.LG) [Total: 12]
- [cond-mat.dis-nn](#cond-mat.dis-nn) [Total: 3]
- [q-bio.PE](#q-bio.PE) [Total: 6]
- [q-bio.NC](#q-bio.NC) [Total: 18]
- [q-bio.CB](#q-bio.CB) [Total: 3]
- [physics.soc-ph](#physics.soc-ph) [Total: 3]
- [q-bio.TO](#q-bio.TO) [Total: 3]
- [cs.CV](#cs.CV) [Total: 3]


<div id='q-bio.GN'></div>

# q-bio.GN [[Back]](#toc)

### [1] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
*Zhizheng Wang,Chi-Ping Day,Chih-Hsuan Wei,Qiao Jin,Robert Leaman,Yifan Yang,Shubo Tian,Aodong Qiu,Yin Fang,Qingqing Zhu,Xinghua Lu,Zhiyong Lu*

Main category: q-bio.GN

TL;DR: cGSA是一种新型AI驱动的基因集分析框架，通过结合上下文感知的路径优先级，解决了传统GSA方法忽略临床背景的问题，提高了结果的精确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 传统GSA方法生成的结果冗余且缺乏临床相关性，导致解释困难且可靠性低。

Method: cGSA整合了基因簇检测、富集分析和大型语言模型，以识别具有统计显著性和生物学意义的路径。

Result: 在102个手动整理的基因集上，cGSA比基线方法性能提升30%以上，专家验证确认其精确性和可解释性。

Conclusion: cGSA在黑色素瘤和乳腺癌的案例研究中展示了其揭示上下文特异性见解和生成针对性假设的潜力。

Abstract: Gene set analysis (GSA) is a foundational approach for interpreting genomic
data of diseases by linking genes to biological processes. However,
conventional GSA methods overlook clinical context of the analyses, often
generating long lists of enriched pathways with redundant, nonspecific, or
irrelevant results. Interpreting these requires extensive, ad-hoc manual
effort, reducing both reliability and reproducibility. To address this
limitation, we introduce cGSA, a novel AI-driven framework that enhances GSA by
incorporating context-aware pathway prioritization. cGSA integrates gene
cluster detection, enrichment analysis, and large language models to identify
pathways that are not only statistically significant but also biologically
meaningful. Benchmarking on 102 manually curated gene sets across 19 diseases
and ten disease-related biological mechanisms shows that cGSA outperforms
baseline methods by over 30%, with expert validation confirming its increased
precision and interpretability. Two independent case studies in melanoma and
breast cancer further demonstrate its potential to uncover context-specific
insights and support targeted hypothesis generation.

</details>


### [2] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
*Zhizheng Wang,Chi-Ping Day,Chih-Hsuan Wei,Qiao Jin,Robert Leaman,Yifan Yang,Shubo Tian,Aodong Qiu,Yin Fang,Qingqing Zhu,Xinghua Lu,Zhiyong Lu*

Main category: q-bio.GN

TL;DR: cGSA是一种新型AI驱动的基因集分析框架，通过上下文感知的路径优先级提升传统GSA方法，解决了冗余和无关结果的问题。


<details>
  <summary>Details</summary>
Motivation: 传统GSA方法忽视临床背景，导致结果冗余且难以解释，降低了可靠性和可重复性。

Method: cGSA整合基因簇检测、富集分析和大型语言模型，识别具有统计显著性和生物学意义的路径。

Result: 在19种疾病和10种疾病相关机制的102个基因集上，cGSA比基线方法性能提升30%以上，专家验证证实其精确性和可解释性。

Conclusion: cGSA在黑色素瘤和乳腺癌的案例中展示了其发现上下文特异性见解和支持针对性假设生成的潜力。

Abstract: Gene set analysis (GSA) is a foundational approach for interpreting genomic
data of diseases by linking genes to biological processes. However,
conventional GSA methods overlook clinical context of the analyses, often
generating long lists of enriched pathways with redundant, nonspecific, or
irrelevant results. Interpreting these requires extensive, ad-hoc manual
effort, reducing both reliability and reproducibility. To address this
limitation, we introduce cGSA, a novel AI-driven framework that enhances GSA by
incorporating context-aware pathway prioritization. cGSA integrates gene
cluster detection, enrichment analysis, and large language models to identify
pathways that are not only statistically significant but also biologically
meaningful. Benchmarking on 102 manually curated gene sets across 19 diseases
and ten disease-related biological mechanisms shows that cGSA outperforms
baseline methods by over 30%, with expert validation confirming its increased
precision and interpretability. Two independent case studies in melanoma and
breast cancer further demonstrate its potential to uncover context-specific
insights and support targeted hypothesis generation.

</details>


### [3] [Knowledge-guided Contextual Gene Set Analysis Using Large Language Models](https://arxiv.org/abs/2506.04303)
*Zhizheng Wang,Chi-Ping Day,Chih-Hsuan Wei,Qiao Jin,Robert Leaman,Yifan Yang,Shubo Tian,Aodong Qiu,Yin Fang,Qingqing Zhu,Xinghua Lu,Zhiyong Lu*

Main category: q-bio.GN

TL;DR: cGSA是一种新型AI驱动的框架，通过结合上下文感知的路径优先级提升基因集分析（GSA），解决了传统GSA方法忽略临床背景的问题。


<details>
  <summary>Details</summary>
Motivation: 传统GSA方法生成冗长、冗余或无关的路径列表，需要大量人工解释，降低了可靠性和可重复性。cGSA旨在解决这一问题。

Method: cGSA整合基因簇检测、富集分析和大型语言模型，识别统计显著且生物学意义明确的路径。

Result: 在102个手动整理的基因集上，cGSA比基线方法性能提升30%以上，专家验证证实其精确性和可解释性。

Conclusion: cGSA在黑色素瘤和乳腺癌案例中展示了其发现上下文特异性见解的潜力，支持针对性假设生成。

Abstract: Gene set analysis (GSA) is a foundational approach for interpreting genomic
data of diseases by linking genes to biological processes. However,
conventional GSA methods overlook clinical context of the analyses, often
generating long lists of enriched pathways with redundant, nonspecific, or
irrelevant results. Interpreting these requires extensive, ad-hoc manual
effort, reducing both reliability and reproducibility. To address this
limitation, we introduce cGSA, a novel AI-driven framework that enhances GSA by
incorporating context-aware pathway prioritization. cGSA integrates gene
cluster detection, enrichment analysis, and large language models to identify
pathways that are not only statistically significant but also biologically
meaningful. Benchmarking on 102 manually curated gene sets across 19 diseases
and ten disease-related biological mechanisms shows that cGSA outperforms
baseline methods by over 30%, with expert validation confirming its increased
precision and interpretability. Two independent case studies in melanoma and
breast cancer further demonstrate its potential to uncover context-specific
insights and support targeted hypothesis generation.

</details>


<div id='q-bio.QM'></div>

# q-bio.QM [[Back]](#toc)

### [4] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
*Xinyan Zhao,Yi-Ching Tang,Akshita Singh,Victor J Cantu,KwanHo An,Junseok Lee,Adam E Stogsdill,Ashwin Kumar Ramesh,Zhiqiang An,Xiaoqian Jiang,Yejin Kim*

Main category: q-bio.QM

TL;DR: AbBiBench是一个抗体结合亲和力成熟与设计的基准框架，通过评估抗体-抗原复合物的功能潜力，系统比较了14种蛋白质模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有抗体评估方法仅关注抗体本身或其与天然抗体的相似性，而AbBiBench旨在提供一个更全面的功能评估框架。

Method: AbBiBench通过标准化9个数据集（包含155,853个重链突变抗体）和14种蛋白质模型，利用模型似然与实验亲和力的相关性评估性能。

Result: 结构条件逆向折叠模型在亲和力相关性和生成任务中表现最佳。

Conclusion: AbBiBench为开发更有效的功能感知抗体设计模型提供了统一的生物学评估框架。

Abstract: We introduce AbBiBench (Antibody Binding Benchmarking), a benchmarking
framework for antibody binding affinity maturation and design. Unlike existing
antibody evaluation strategies that rely on antibody alone and its similarity
to natural ones (e.g., amino acid identity rate, structural RMSD), AbBiBench
considers an antibody-antigen (Ab-Ag) complex as a functional unit and
evaluates the potential of an antibody design binding to given antigen by
measuring protein model's likelihood on the Ab-Ag complex. We first curate,
standardize, and share 9 datasets containing 9 antigens (involving influenza,
anti-lysozyme, HER2, VEGF, integrin, and SARS-CoV-2) and 155,853 heavy chain
mutated antibodies. Using these datasets, we systematically compare 14 protein
models including masked language models, autoregressive language models,
inverse folding models, diffusion-based generative models, and geometric graph
models. The correlation between model likelihood and experimental affinity
values is used to evaluate model performance. Additionally, in a case study to
increase binding affinity of antibody F045-092 to antigen influenza H1N1, we
evaluate the generative power of the top-performing models by sampling a set of
new antibodies binding to the antigen and ranking them based on structural
integrity and biophysical properties of the Ab-Ag complex. As a result,
structure-conditioned inverse folding models outperform others in both affinity
correlation and generation tasks. Overall, AbBiBench provides a unified,
biologically grounded evaluation framework to facilitate the development of
more effective, function-aware antibody design models.

</details>


### [5] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TL;DR: 论文提出了一种基于潜在空间假设的医学数据分析框架，将多模态医学数据映射到统一的学习几何表示中，以支持个性化诊断和治疗。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且复杂，但可能共享相同的生理状态信息。潜在空间假设旨在统一这些数据，揭示疾病进展和治疗的几何表示。

Method: 通过潜在空间假设，将多模态医学数据映射到统一的学习几何表示中，利用轨迹和向量描述疾病进展和治疗干预。

Result: 该框架能够揭示疾病的子轨迹和患者特异性变化方向，为个性化医疗提供定量依据。

Conclusion: 潜在空间假设为个性化医疗提供了新思路，但仍需解决数据偏差、稀缺性和隐私等挑战。

Abstract: Medical data range from genomic sequences and retinal photographs to
structured laboratory results and unstructured clinical narratives. Although
these modalities appear disparate, many encode convergent information about a
single underlying physiological state. The Latent Space Hypothesis frames each
observation as a projection of a unified, hierarchically organized manifold --
much like shadows cast by the same three-dimensional object. Within this
learned geometric representation, an individual's health status occupies a
point, disease progression traces a trajectory, and therapeutic intervention
corresponds to a directed vector. Interpreting heterogeneous evidence in a
shared space provides a principled way to re-examine eponymous conditions --
such as Parkinson's or Crohn's -- that often mask multiple pathophysiological
entities and involve broader anatomical domains than once believed. By
revealing sub-trajectories and patient-specific directions of change, the
framework supplies a quantitative rationale for personalised diagnosis,
longitudinal monitoring, and tailored treatment, moving clinical practice away
from grouping by potentially misleading labels toward navigation of each
person's unique trajectory. Challenges remain -- bias amplification, data
scarcity for rare disorders, privacy, and the correlation-causation divide --
but scale-aware encoders, continual learning on longitudinal data streams, and
perturbation-based validation offer plausible paths forward.

</details>


### [6] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
*Xinyan Zhao,Yi-Ching Tang,Akshita Singh,Victor J Cantu,KwanHo An,Junseok Lee,Adam E Stogsdill,Ashwin Kumar Ramesh,Zhiqiang An,Xiaoqian Jiang,Yejin Kim*

Main category: q-bio.QM

TL;DR: AbBiBench是一个抗体结合亲和力成熟与设计的基准框架，通过评估抗体-抗原复合物的功能单元来改进现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有抗体评估方法仅关注抗体本身或其与天然抗体的相似性，缺乏对抗体-抗原复合物的功能评估。

Method: AbBiBench通过测量蛋白模型在抗体-抗原复合物上的似然性来评估抗体设计的潜力，并比较了14种蛋白模型的性能。

Result: 结构条件逆向折叠模型在亲和力相关性和生成任务中表现最佳。

Conclusion: AbBiBench提供了一个统一的、基于生物学的评估框架，促进更有效的功能感知抗体设计模型的开发。

Abstract: We introduce AbBiBench (Antibody Binding Benchmarking), a benchmarking
framework for antibody binding affinity maturation and design. Unlike existing
antibody evaluation strategies that rely on antibody alone and its similarity
to natural ones (e.g., amino acid identity rate, structural RMSD), AbBiBench
considers an antibody-antigen (Ab-Ag) complex as a functional unit and
evaluates the potential of an antibody design binding to given antigen by
measuring protein model's likelihood on the Ab-Ag complex. We first curate,
standardize, and share 9 datasets containing 9 antigens (involving influenza,
anti-lysozyme, HER2, VEGF, integrin, and SARS-CoV-2) and 155,853 heavy chain
mutated antibodies. Using these datasets, we systematically compare 14 protein
models including masked language models, autoregressive language models,
inverse folding models, diffusion-based generative models, and geometric graph
models. The correlation between model likelihood and experimental affinity
values is used to evaluate model performance. Additionally, in a case study to
increase binding affinity of antibody F045-092 to antigen influenza H1N1, we
evaluate the generative power of the top-performing models by sampling a set of
new antibodies binding to the antigen and ranking them based on structural
integrity and biophysical properties of the Ab-Ag complex. As a result,
structure-conditioned inverse folding models outperform others in both affinity
correlation and generation tasks. Overall, AbBiBench provides a unified,
biologically grounded evaluation framework to facilitate the development of
more effective, function-aware antibody design models.

</details>


### [7] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TL;DR: 论文提出了一种基于潜在空间假设的框架，将多模态医学数据映射到统一的学习几何表示中，用于个性化医疗。


<details>
  <summary>Details</summary>
Motivation: 解决医学数据多模态但信息收敛的问题，重新审视传统疾病分类的局限性，推动个性化医疗。

Method: 采用潜在空间假设，将医学数据映射到统一的学习几何表示中，通过轨迹和向量表示健康状态、疾病进展和治疗干预。

Result: 揭示了疾病亚轨迹和患者特异性变化方向，为个性化诊断、监测和治疗提供了量化依据。

Conclusion: 该框架为个性化医疗提供了新思路，但仍需解决数据偏见、稀缺性、隐私和因果推断等挑战。

Abstract: Medical data range from genomic sequences and retinal photographs to
structured laboratory results and unstructured clinical narratives. Although
these modalities appear disparate, many encode convergent information about a
single underlying physiological state. The Latent Space Hypothesis frames each
observation as a projection of a unified, hierarchically organized manifold --
much like shadows cast by the same three-dimensional object. Within this
learned geometric representation, an individual's health status occupies a
point, disease progression traces a trajectory, and therapeutic intervention
corresponds to a directed vector. Interpreting heterogeneous evidence in a
shared space provides a principled way to re-examine eponymous conditions --
such as Parkinson's or Crohn's -- that often mask multiple pathophysiological
entities and involve broader anatomical domains than once believed. By
revealing sub-trajectories and patient-specific directions of change, the
framework supplies a quantitative rationale for personalised diagnosis,
longitudinal monitoring, and tailored treatment, moving clinical practice away
from grouping by potentially misleading labels toward navigation of each
person's unique trajectory. Challenges remain -- bias amplification, data
scarcity for rare disorders, privacy, and the correlation-causation divide --
but scale-aware encoders, continual learning on longitudinal data streams, and
perturbation-based validation offer plausible paths forward.

</details>


### [8] [Benchmark for Antibody Binding Affinity Maturation and Design](https://arxiv.org/abs/2506.04235)
*Xinyan Zhao,Yi-Ching Tang,Akshita Singh,Victor J Cantu,KwanHo An,Junseok Lee,Adam E Stogsdill,Ashwin Kumar Ramesh,Zhiqiang An,Xiaoqian Jiang,Yejin Kim*

Main category: q-bio.QM

TL;DR: AbBiBench是一个抗体结合亲和力成熟和设计的基准框架，通过评估抗体-抗原复合物的功能潜力，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有抗体评估方法仅关注抗体本身或其与天然抗体的相似性，而AbBiBench旨在通过抗体-抗原复合物的功能评估，提供更全面的设计模型。

Method: AbBiBench使用9个数据集（包含9种抗原和155,853个重链突变抗体），比较14种蛋白质模型的性能，并通过模型似然与实验亲和力值的相关性评估模型。

Result: 结构条件逆向折叠模型在亲和力相关性和生成任务中表现最佳。

Conclusion: AbBiBench提供了一个统一且生物学基础的评价框架，有助于开发更有效的功能感知抗体设计模型。

Abstract: We introduce AbBiBench (Antibody Binding Benchmarking), a benchmarking
framework for antibody binding affinity maturation and design. Unlike existing
antibody evaluation strategies that rely on antibody alone and its similarity
to natural ones (e.g., amino acid identity rate, structural RMSD), AbBiBench
considers an antibody-antigen (Ab-Ag) complex as a functional unit and
evaluates the potential of an antibody design binding to given antigen by
measuring protein model's likelihood on the Ab-Ag complex. We first curate,
standardize, and share 9 datasets containing 9 antigens (involving influenza,
anti-lysozyme, HER2, VEGF, integrin, and SARS-CoV-2) and 155,853 heavy chain
mutated antibodies. Using these datasets, we systematically compare 14 protein
models including masked language models, autoregressive language models,
inverse folding models, diffusion-based generative models, and geometric graph
models. The correlation between model likelihood and experimental affinity
values is used to evaluate model performance. Additionally, in a case study to
increase binding affinity of antibody F045-092 to antigen influenza H1N1, we
evaluate the generative power of the top-performing models by sampling a set of
new antibodies binding to the antigen and ranking them based on structural
integrity and biophysical properties of the Ab-Ag complex. As a result,
structure-conditioned inverse folding models outperform others in both affinity
correlation and generation tasks. Overall, AbBiBench provides a unified,
biologically grounded evaluation framework to facilitate the development of
more effective, function-aware antibody design models.

</details>


### [9] [The Latent Space Hypothesis: Toward Universal Medical Representation Learning](https://arxiv.org/abs/2506.04515)
*Salil Patel*

Main category: q-bio.QM

TL;DR: 论文提出了一种基于潜在空间假设的医学数据分析框架，将多模态医学数据映射到统一的空间中，以揭示个体健康状态、疾病进展和治疗干预的几何表示。


<details>
  <summary>Details</summary>
Motivation: 医学数据多模态且复杂，但可能包含关于同一生理状态的收敛信息。传统方法可能掩盖疾病的异质性，需要一种统一框架来支持个性化医疗。

Method: 通过潜在空间假设，将多模态数据投影到一个统一的分层流形中，利用几何表示分析健康状态、疾病轨迹和治疗向量。

Result: 该框架能够揭示疾病的子轨迹和患者特异性变化方向，为个性化诊断、监测和治疗提供定量依据。

Conclusion: 潜在空间假设为医学数据分析提供了新视角，但仍需解决数据偏见、稀缺性、隐私和因果推断等挑战。

Abstract: Medical data range from genomic sequences and retinal photographs to
structured laboratory results and unstructured clinical narratives. Although
these modalities appear disparate, many encode convergent information about a
single underlying physiological state. The Latent Space Hypothesis frames each
observation as a projection of a unified, hierarchically organized manifold --
much like shadows cast by the same three-dimensional object. Within this
learned geometric representation, an individual's health status occupies a
point, disease progression traces a trajectory, and therapeutic intervention
corresponds to a directed vector. Interpreting heterogeneous evidence in a
shared space provides a principled way to re-examine eponymous conditions --
such as Parkinson's or Crohn's -- that often mask multiple pathophysiological
entities and involve broader anatomical domains than once believed. By
revealing sub-trajectories and patient-specific directions of change, the
framework supplies a quantitative rationale for personalised diagnosis,
longitudinal monitoring, and tailored treatment, moving clinical practice away
from grouping by potentially misleading labels toward navigation of each
person's unique trajectory. Challenges remain -- bias amplification, data
scarcity for rare disorders, privacy, and the correlation-causation divide --
but scale-aware encoders, continual learning on longitudinal data streams, and
perturbation-based validation offer plausible paths forward.

</details>


<div id='eess.IV'></div>

# eess.IV [[Back]](#toc)

### [10] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
*Srikar Yellapragada,Alexandros Graikos,Zilinghan Li,Kostas Triaridis,Varun Belagali,Saarthak Kapse,Tarak Nath Nandi,Ravi K Madduri,Prateek Prasanna,Tahsin Kurc,Rajarsi R. Gupta,Joel Saltz,Dimitris Samaras*

Main category: eess.IV

TL;DR: PixCell是一种基于扩散的生成基础模型，用于组织病理学，能够在无标注数据的情况下生成高质量图像，解决数据稀缺和隐私问题。


<details>
  <summary>Details</summary>
Motivation: 解决病理学中数据稀缺、隐私保护和生成任务（如虚拟染色）的挑战。

Method: 使用扩散模型PixCell，在PanCan-30M数据集上进行渐进式训练，结合自监督条件。

Result: 生成多样且高质量的图像，可用于训练自监督模型、数据增强和教育，并实现分子标记推断。

Conclusion: PixCell为计算病理学提供了强大的生成工具，加速研究并解决实际问题。

Abstract: The digitization of histology slides has revolutionized pathology, providing
massive datasets for cancer diagnosis and research. Contrastive self-supervised
and vision-language models have been shown to effectively mine large pathology
datasets to learn discriminative representations. On the other hand, generative
models, capable of synthesizing realistic and diverse images, present a
compelling solution to address unique problems in pathology that involve
synthesizing images; overcoming annotated data scarcity, enabling
privacy-preserving data sharing, and performing inherently generative tasks,
such as virtual staining. We introduce PixCell, the first diffusion-based
generative foundation model for histopathology. We train PixCell on PanCan-30M,
a vast, diverse dataset derived from 69,184 H\&E-stained whole slide images
covering various cancer types. We employ a progressive training strategy and a
self-supervision-based conditioning that allows us to scale up training without
any annotated data. PixCell generates diverse and high-quality images across
multiple cancer types, which we find can be used in place of real data to train
a self-supervised discriminative model. Synthetic images shared between
institutions are subject to fewer regulatory barriers than would be the case
with real clinical images. Furthermore, we showcase the ability to precisely
control image generation using a small set of annotated images, which can be
used for both data augmentation and educational purposes. Testing on a cell
segmentation task, a mask-guided PixCell enables targeted data augmentation,
improving downstream performance. Finally, we demonstrate PixCell's ability to
use H\&E structural staining to infer results from molecular marker studies; we
use this capability to infer IHC staining from H\&E images. Our trained models
are publicly released to accelerate research in computational pathology.

</details>


### [11] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
*Srikar Yellapragada,Alexandros Graikos,Zilinghan Li,Kostas Triaridis,Varun Belagali,Saarthak Kapse,Tarak Nath Nandi,Ravi K Madduri,Prateek Prasanna,Tahsin Kurc,Rajarsi R. Gupta,Joel Saltz,Dimitris Samaras*

Main category: eess.IV

TL;DR: PixCell是一种基于扩散模型的生成基础模型，用于组织病理学，能够生成高质量图像，解决数据稀缺和隐私问题，并支持数据增强和教育用途。


<details>
  <summary>Details</summary>
Motivation: 解决组织病理学中数据稀缺、隐私保护和生成任务（如虚拟染色）的挑战。

Method: 使用扩散模型PixCell，在PanCan-30M数据集上进行渐进式训练，无需标注数据，通过自监督条件控制生成。

Result: 生成多样且高质量的图像，可用于训练判别模型、数据增强和分子标记推断（如从H&E图像推断IHC染色）。

Conclusion: PixCell为计算病理学研究提供了强大的生成工具，支持数据共享和下游任务性能提升。

Abstract: The digitization of histology slides has revolutionized pathology, providing
massive datasets for cancer diagnosis and research. Contrastive self-supervised
and vision-language models have been shown to effectively mine large pathology
datasets to learn discriminative representations. On the other hand, generative
models, capable of synthesizing realistic and diverse images, present a
compelling solution to address unique problems in pathology that involve
synthesizing images; overcoming annotated data scarcity, enabling
privacy-preserving data sharing, and performing inherently generative tasks,
such as virtual staining. We introduce PixCell, the first diffusion-based
generative foundation model for histopathology. We train PixCell on PanCan-30M,
a vast, diverse dataset derived from 69,184 H\&E-stained whole slide images
covering various cancer types. We employ a progressive training strategy and a
self-supervision-based conditioning that allows us to scale up training without
any annotated data. PixCell generates diverse and high-quality images across
multiple cancer types, which we find can be used in place of real data to train
a self-supervised discriminative model. Synthetic images shared between
institutions are subject to fewer regulatory barriers than would be the case
with real clinical images. Furthermore, we showcase the ability to precisely
control image generation using a small set of annotated images, which can be
used for both data augmentation and educational purposes. Testing on a cell
segmentation task, a mask-guided PixCell enables targeted data augmentation,
improving downstream performance. Finally, we demonstrate PixCell's ability to
use H\&E structural staining to infer results from molecular marker studies; we
use this capability to infer IHC staining from H\&E images. Our trained models
are publicly released to accelerate research in computational pathology.

</details>


### [12] [PixCell: A generative foundation model for digital histopathology images](https://arxiv.org/abs/2506.05127)
*Srikar Yellapragada,Alexandros Graikos,Zilinghan Li,Kostas Triaridis,Varun Belagali,Saarthak Kapse,Tarak Nath Nandi,Ravi K Madduri,Prateek Prasanna,Tahsin Kurc,Rajarsi R. Gupta,Joel Saltz,Dimitris Samaras*

Main category: eess.IV

TL;DR: PixCell是一种基于扩散模型的生成基础模型，用于组织病理学，能够生成高质量图像，解决数据稀缺和隐私问题，并支持数据增强和教育应用。


<details>
  <summary>Details</summary>
Motivation: 解决病理学中数据稀缺、隐私保护和生成任务（如虚拟染色）的挑战。

Method: 使用扩散模型和自监督条件训练PixCell，基于PanCan-30M数据集，无需标注数据。

Result: PixCell生成多样且高质量的图像，可用于训练自监督模型、数据增强和分子标记推断。

Conclusion: PixCell为计算病理学提供了强大的生成工具，加速研究并解决实际问题。

Abstract: The digitization of histology slides has revolutionized pathology, providing
massive datasets for cancer diagnosis and research. Contrastive self-supervised
and vision-language models have been shown to effectively mine large pathology
datasets to learn discriminative representations. On the other hand, generative
models, capable of synthesizing realistic and diverse images, present a
compelling solution to address unique problems in pathology that involve
synthesizing images; overcoming annotated data scarcity, enabling
privacy-preserving data sharing, and performing inherently generative tasks,
such as virtual staining. We introduce PixCell, the first diffusion-based
generative foundation model for histopathology. We train PixCell on PanCan-30M,
a vast, diverse dataset derived from 69,184 H\&E-stained whole slide images
covering various cancer types. We employ a progressive training strategy and a
self-supervision-based conditioning that allows us to scale up training without
any annotated data. PixCell generates diverse and high-quality images across
multiple cancer types, which we find can be used in place of real data to train
a self-supervised discriminative model. Synthetic images shared between
institutions are subject to fewer regulatory barriers than would be the case
with real clinical images. Furthermore, we showcase the ability to precisely
control image generation using a small set of annotated images, which can be
used for both data augmentation and educational purposes. Testing on a cell
segmentation task, a mask-guided PixCell enables targeted data augmentation,
improving downstream performance. Finally, we demonstrate PixCell's ability to
use H\&E structural staining to infer results from molecular marker studies; we
use this capability to infer IHC staining from H\&E images. Our trained models
are publicly released to accelerate research in computational pathology.

</details>


<div id='cs.LG'></div>

# cs.LG [[Back]](#toc)

### [13] [Relational reasoning and inductive bias in transformers trained on a transitive inference task](https://arxiv.org/abs/2506.04289)
*Jesse Geerts,Stephanie Chan,Claudia Clopath,Kimberly Stachenfeld*

Main category: cs.LG

TL;DR: 论文研究了Transformer在不同学习机制下（参数内学习与上下文学习）处理传递推理任务的能力，发现参数内学习自然支持传递推理，而上下文学习需特定预训练才能实现。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer在传递推理任务中的表现机制，比较不同学习机制（参数内学习与上下文学习）的差异。

Method: 通过实验比较参数内学习（IWL）和上下文学习（ICL）在传递推理任务中的表现，并进行机制分析。

Result: IWL自然支持传递推理，而ICL需预训练才能实现。预训练后，ICL模型表现出类似人类的推理特征。

Conclusion: 预训练具有结构化任务可促进上下文学习中的关系推理能力。

Abstract: Transformer-based models have demonstrated remarkable reasoning abilities,
but the mechanisms underlying relational reasoning in different learning
regimes remain poorly understood. In this work, we investigate how transformers
perform a classic relational reasoning task from the Psychology literature,
\textit{transitive inference}, which requires inference about indirectly
related items by integrating information across observed adjacent item pairs
(e.g., if A>B and B>C, then A>C). We compare transitive inference behavior
across two distinct learning regimes: in-weights learning (IWL), where models
store information in network parameters, and in-context learning (ICL), where
models flexibly utilize information presented within the input sequence. Our
findings reveal that IWL naturally induces a generalization bias towards
transitive inference, despite being trained only on adjacent items, whereas ICL
models trained solely on adjacent items do not generalize transitively.
Mechanistic analysis shows that ICL models develop induction circuits that
implement a simple match-and-copy strategy that performs well at relating
adjacent pairs, but does not encoding hierarchical relationships among
indirectly related items. Interestingly, when pre-trained on in-context linear
regression tasks, transformers successfully exhibit in-context generalizable
transitive inference. Moreover, like IWL, they display both \textit{symbolic
distance} and \textit{terminal item effects} characteristic of human and animal
performance, without forming induction circuits. These results suggest that
pre-training on tasks with underlying structure promotes the development of
representations that can scaffold in-context relational reasoning.

</details>


### [14] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: 论文提出了一种结合冷冻电镜密度图和蛋白质结构预测模型的方法CryoBoltz，用于生成动态生物分子复合物的多种构象状态。


<details>
  <summary>Details</summary>
Motivation: 当前蛋白质结构预测模型难以捕捉动态生物分子复合物的构象多样性，而冷冻电镜虽能成像结构异质性，但数据处理流程复杂。

Method: CryoBoltz利用冷冻电镜密度图的全局和局部结构约束，引导预训练蛋白质结构预测模型的采样轨迹，生成与实验数据一致的构象状态。

Result: 该方法成功应用于多种动态生物分子系统（如转运蛋白和抗体），构建了原子模型。

Conclusion: CryoBoltz为动态生物分子复合物的结构预测提供了一种灵活且强大的解决方案。

Abstract: Protein structure prediction models are now capable of generating accurate 3D
structural hypotheses from sequence alone. However, they routinely fail to
capture the conformational diversity of dynamic biomolecular complexes, often
requiring heuristic MSA subsampling approaches for generating alternative
states. In parallel, cryo-electron microscopy (cryo-EM) has emerged as a
powerful tool for imaging near-native structural heterogeneity, but is
challenged by arduous pipelines to go from raw experimental data to atomic
models. Here, we bridge the gap between these modalities, combining cryo-EM
density maps with the rich sequence and biophysical priors learned by protein
structure prediction models. Our method, CryoBoltz, guides the sampling
trajectory of a pretrained protein structure prediction model using both global
and local structural constraints derived from density maps, driving predictions
towards conformational states consistent with the experimental data. We
demonstrate that this flexible yet powerful inference-time approach allows us
to build atomic models into heterogeneous cryo-EM maps across a variety of
dynamic biomolecular systems including transporters and antibodies.

</details>


### [15] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE是一个神经算子框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统生物神经元模型在捕捉变异性和计算效率上的不足。


<details>
  <summary>Details</summary>
Motivation: 当前神经元模型受限于实验数据的有限性和变异性，传统方法难以扩展，深度学习又无法完全捕捉生物物理复杂性。

Method: NOBLE利用深度学习框架，从神经元特征嵌入预测电压响应分布，并验证于真实实验数据。

Result: NOBLE实现了4200倍的速度提升，生成具有实验变异性的合成神经元，并捕捉了基本神经特性。

Conclusion: NOBLE为理解神经元组成、计算及神经AI应用提供了新途径。

Abstract: Characterizing the diverse computational properties of human neurons via
multimodal electrophysiological, transcriptomic, and morphological data
provides the foundation for constructing and validating bio-realistic neuron
models that can advance our understanding of fundamental mechanisms underlying
brain function. However, current modeling approaches remain constrained by the
limited availability and intrinsic variability of experimental neuronal data.
To capture variability, ensembles of deterministic models are often used, but
are difficult to scale as model generation requires repeating computationally
expensive optimization for each neuron. While deep learning is becoming
increasingly relevant in this space, it fails to capture the full biophysical
complexity of neurons, their nonlinear voltage dynamics, and variability. To
address these shortcomings, we introduce NOBLE, a neural operator framework
that learns a mapping from a continuous frequency-modulated embedding of
interpretable neuron features to the somatic voltage response induced by
current injection. Trained on data generated from biophysically realistic
neuron models, NOBLE predicts distributions of neural dynamics accounting for
the intrinsic experimental variability. Unlike conventional bio-realistic
neuron models, interpolating within the embedding space offers models whose
dynamics are consistent with experimentally observed responses. NOBLE is the
first scaled-up deep learning framework validated on real experimental data,
enabling efficient generation of synthetic neurons that exhibit trial-to-trial
variability and achieve a $4200\times$ speedup over numerical solvers. To this
end, NOBLE captures fundamental neural properties, opening the door to a better
understanding of cellular composition and computations, neuromorphic
architectures, large-scale brain circuits, and general neuroAI applications.

</details>


### [16] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TL;DR: 研究了生成扩散模型与连续状态联想记忆模型之间的联系，强调了Morse-Smale动力系统作为梯度联想记忆模型的通用逼近器，并探讨了扩散模型作为其白噪声扰动系统的性质。


<details>
  <summary>Details</summary>
Motivation: 探索生成扩散模型与联想记忆模型之间的理论联系，揭示其通用性质及噪声水平变化对系统行为的影响。

Method: 利用Morse-Smale动力系统作为理论框架，分析梯度联想记忆模型和扩散模型的稳定性及分岔行为。

Result: 发现联想记忆和扩散模型在噪声消失时具有稳定性，且学习景观中仅存在孤立的不稳定点。通过分岔序列刻画了系统的定性转变。

Conclusion: 研究为理解生成与记忆之间的过渡提供了理论支持，并验证了经典和现代Hopfield网络的结构稳定性。

Abstract: Connections between generative diffusion and continuous-state associative
memory models are studied. Morse-Smale dynamical systems are emphasized as
universal approximators of gradient-based associative memory models and
diffusion models as white-noise perturbed systems thereof. Universal properties
of associative memory that follow from this description are described and used
to characterize a generic transition from generation to memory as noise levels
diminish. Structural stability inherited by Morse-Smale flows is shown to imply
a notion of stability for diffusions at vanishing noise levels. Applied to one-
and two-parameter families of gradients, this indicates stability at all but
isolated points of associative memory learning landscapes and the learning and
generation landscapes of diffusion models with gradient drift in the zero-noise
limit, at which small sets of generic bifurcations characterize qualitative
transitions between stable systems. Examples illustrating the characterization
of these landscapes by sequences of these bifurcations are given, along with
structural stability criterion for classic and modern Hopfield networks
(equivalently, the attention mechanism).

</details>


### [17] [Relational reasoning and inductive bias in transformers trained on a transitive inference task](https://arxiv.org/abs/2506.04289)
*Jesse Geerts,Stephanie Chan,Claudia Clopath,Kimberly Stachenfeld*

Main category: cs.LG

TL;DR: 论文研究了Transformer模型在关系推理任务中的表现，比较了两种学习机制（IWL和ICL）在传递推理任务中的差异，发现预训练任务的结构性有助于提升上下文关系推理能力。


<details>
  <summary>Details</summary>
Motivation: 探索Transformer模型在不同学习机制下（参数内学习IWL和上下文学习ICL）如何完成经典的传递推理任务，以理解其关系推理的机制。

Method: 通过比较IWL和ICL在传递推理任务中的表现，分析其机制，并测试预训练任务对上下文推理能力的影响。

Result: IWL模型能自然泛化传递推理，而ICL模型仅能处理相邻关系。预训练后，ICL模型也能实现传递推理，并表现出类似人类的行为特征。

Conclusion: 预训练任务的结构性有助于模型发展支持上下文关系推理的表征能力。

Abstract: Transformer-based models have demonstrated remarkable reasoning abilities,
but the mechanisms underlying relational reasoning in different learning
regimes remain poorly understood. In this work, we investigate how transformers
perform a classic relational reasoning task from the Psychology literature,
\textit{transitive inference}, which requires inference about indirectly
related items by integrating information across observed adjacent item pairs
(e.g., if A>B and B>C, then A>C). We compare transitive inference behavior
across two distinct learning regimes: in-weights learning (IWL), where models
store information in network parameters, and in-context learning (ICL), where
models flexibly utilize information presented within the input sequence. Our
findings reveal that IWL naturally induces a generalization bias towards
transitive inference, despite being trained only on adjacent items, whereas ICL
models trained solely on adjacent items do not generalize transitively.
Mechanistic analysis shows that ICL models develop induction circuits that
implement a simple match-and-copy strategy that performs well at relating
adjacent pairs, but does not encoding hierarchical relationships among
indirectly related items. Interestingly, when pre-trained on in-context linear
regression tasks, transformers successfully exhibit in-context generalizable
transitive inference. Moreover, like IWL, they display both \textit{symbolic
distance} and \textit{terminal item effects} characteristic of human and animal
performance, without forming induction circuits. These results suggest that
pre-training on tasks with underlying structure promotes the development of
representations that can scaffold in-context relational reasoning.

</details>


### [18] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: CryoBoltz结合冷冻电镜密度图与蛋白质结构预测模型，生成与实验数据一致的构象多样性模型。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质结构预测模型无法捕捉动态生物分子复合物构象多样性的问题，同时简化冷冻电镜数据到原子模型的流程。

Method: 利用冷冻电镜密度图的全局和局部结构约束，引导预训练蛋白质结构预测模型的采样轨迹。

Result: 成功构建了多种动态生物分子系统（如转运蛋白和抗体）的原子模型。

Conclusion: CryoBoltz为动态生物分子系统的结构建模提供了一种灵活且强大的方法。

Abstract: Protein structure prediction models are now capable of generating accurate 3D
structural hypotheses from sequence alone. However, they routinely fail to
capture the conformational diversity of dynamic biomolecular complexes, often
requiring heuristic MSA subsampling approaches for generating alternative
states. In parallel, cryo-electron microscopy (cryo-EM) has emerged as a
powerful tool for imaging near-native structural heterogeneity, but is
challenged by arduous pipelines to go from raw experimental data to atomic
models. Here, we bridge the gap between these modalities, combining cryo-EM
density maps with the rich sequence and biophysical priors learned by protein
structure prediction models. Our method, CryoBoltz, guides the sampling
trajectory of a pretrained protein structure prediction model using both global
and local structural constraints derived from density maps, driving predictions
towards conformational states consistent with the experimental data. We
demonstrate that this flexible yet powerful inference-time approach allows us
to build atomic models into heterogeneous cryo-EM maps across a variety of
dynamic biomolecular systems including transporters and antibodies.

</details>


### [19] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE是一种神经操作框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统生物真实神经元模型的局限性和变异性问题。


<details>
  <summary>Details</summary>
Motivation: 当前神经元建模方法受限于实验数据的有限性和变异性，难以捕捉神经元的生物物理复杂性和非线性动态。

Method: NOBLE通过深度学习框架，从神经元特征的连续嵌入中预测电压响应分布，并利用生物物理真实模型生成的数据进行训练。

Result: NOBLE能够高效生成具有实验变异性的人工神经元，速度比数值求解器快4200倍，并验证了其在实际实验数据上的有效性。

Conclusion: NOBLE为理解神经元组成、计算及神经AI应用提供了新工具，推动了大规模脑电路和神经形态架构的研究。

Abstract: Characterizing the diverse computational properties of human neurons via
multimodal electrophysiological, transcriptomic, and morphological data
provides the foundation for constructing and validating bio-realistic neuron
models that can advance our understanding of fundamental mechanisms underlying
brain function. However, current modeling approaches remain constrained by the
limited availability and intrinsic variability of experimental neuronal data.
To capture variability, ensembles of deterministic models are often used, but
are difficult to scale as model generation requires repeating computationally
expensive optimization for each neuron. While deep learning is becoming
increasingly relevant in this space, it fails to capture the full biophysical
complexity of neurons, their nonlinear voltage dynamics, and variability. To
address these shortcomings, we introduce NOBLE, a neural operator framework
that learns a mapping from a continuous frequency-modulated embedding of
interpretable neuron features to the somatic voltage response induced by
current injection. Trained on data generated from biophysically realistic
neuron models, NOBLE predicts distributions of neural dynamics accounting for
the intrinsic experimental variability. Unlike conventional bio-realistic
neuron models, interpolating within the embedding space offers models whose
dynamics are consistent with experimentally observed responses. NOBLE is the
first scaled-up deep learning framework validated on real experimental data,
enabling efficient generation of synthetic neurons that exhibit trial-to-trial
variability and achieve a $4200\times$ speedup over numerical solvers. To this
end, NOBLE captures fundamental neural properties, opening the door to a better
understanding of cellular composition and computations, neuromorphic
architectures, large-scale brain circuits, and general neuroAI applications.

</details>


### [20] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TL;DR: 研究了生成扩散模型与连续状态联想记忆模型之间的联系，强调Morse-Smale动力系统作为梯度联想记忆和扩散模型的通用近似器，并描述了噪声水平降低时从生成到记忆的通用转变。


<details>
  <summary>Details</summary>
Motivation: 探索生成扩散模型与联想记忆模型之间的理论联系，揭示其在噪声变化下的动态行为。

Method: 利用Morse-Smale动力系统作为桥梁，分析梯度联想记忆和扩散模型的结构稳定性，并通过分岔理论描述学习与生成景观的稳定性。

Result: 表明在零噪声极限下，扩散模型和联想记忆模型具有结构稳定性，仅在某些孤立点出现分岔现象。

Conclusion: 通过分岔序列和结构稳定性准则，为经典和现代Hopfield网络（注意力机制）提供了理论支持。

Abstract: Connections between generative diffusion and continuous-state associative
memory models are studied. Morse-Smale dynamical systems are emphasized as
universal approximators of gradient-based associative memory models and
diffusion models as white-noise perturbed systems thereof. Universal properties
of associative memory that follow from this description are described and used
to characterize a generic transition from generation to memory as noise levels
diminish. Structural stability inherited by Morse-Smale flows is shown to imply
a notion of stability for diffusions at vanishing noise levels. Applied to one-
and two-parameter families of gradients, this indicates stability at all but
isolated points of associative memory learning landscapes and the learning and
generation landscapes of diffusion models with gradient drift in the zero-noise
limit, at which small sets of generic bifurcations characterize qualitative
transitions between stable systems. Examples illustrating the characterization
of these landscapes by sequences of these bifurcations are given, along with
structural stability criterion for classic and modern Hopfield networks
(equivalently, the attention mechanism).

</details>


### [21] [Relational reasoning and inductive bias in transformers trained on a transitive inference task](https://arxiv.org/abs/2506.04289)
*Jesse Geerts,Stephanie Chan,Claudia Clopath,Kimberly Stachenfeld*

Main category: cs.LG

TL;DR: 论文研究了Transformer模型在关系推理任务中的表现，比较了两种学习机制（IWL和ICL）在传递性推理中的差异，发现预训练任务的结构性有助于提升上下文关系推理能力。


<details>
  <summary>Details</summary>
Motivation: 探讨Transformer模型在不同学习机制下如何进行关系推理，特别是传递性推理任务，以理解其内在机制。

Method: 通过比较IWL（参数内学习）和ICL（上下文学习）两种机制，分析它们在传递性推理任务中的表现，并进行机制性分析。

Result: IWL自然倾向于传递性推理，而ICL仅适用于相邻项关系推理。预训练任务的结构性有助于ICL模型实现传递性推理。

Conclusion: 预训练任务的结构性可以促进模型形成支持上下文关系推理的表征，而无需依赖特定的推理电路。

Abstract: Transformer-based models have demonstrated remarkable reasoning abilities,
but the mechanisms underlying relational reasoning in different learning
regimes remain poorly understood. In this work, we investigate how transformers
perform a classic relational reasoning task from the Psychology literature,
\textit{transitive inference}, which requires inference about indirectly
related items by integrating information across observed adjacent item pairs
(e.g., if A>B and B>C, then A>C). We compare transitive inference behavior
across two distinct learning regimes: in-weights learning (IWL), where models
store information in network parameters, and in-context learning (ICL), where
models flexibly utilize information presented within the input sequence. Our
findings reveal that IWL naturally induces a generalization bias towards
transitive inference, despite being trained only on adjacent items, whereas ICL
models trained solely on adjacent items do not generalize transitively.
Mechanistic analysis shows that ICL models develop induction circuits that
implement a simple match-and-copy strategy that performs well at relating
adjacent pairs, but does not encoding hierarchical relationships among
indirectly related items. Interestingly, when pre-trained on in-context linear
regression tasks, transformers successfully exhibit in-context generalizable
transitive inference. Moreover, like IWL, they display both \textit{symbolic
distance} and \textit{terminal item effects} characteristic of human and animal
performance, without forming induction circuits. These results suggest that
pre-training on tasks with underlying structure promotes the development of
representations that can scaffold in-context relational reasoning.

</details>


### [22] [Multiscale guidance of AlphaFold3 with heterogeneous cryo-EM data](https://arxiv.org/abs/2506.04490)
*Rishwanth Raghu,Axel Levy,Gordon Wetzstein,Ellen D. Zhong*

Main category: cs.LG

TL;DR: CryoBoltz结合冷冻电镜密度图和蛋白质结构预测模型，通过全局和局部结构约束指导采样，生成与实验数据一致的构象状态。


<details>
  <summary>Details</summary>
Motivation: 解决蛋白质结构预测模型在捕捉动态生物分子复合物构象多样性上的不足，同时简化冷冻电镜从原始数据到原子模型的流程。

Method: 利用冷冻电镜密度图和预训练的蛋白质结构预测模型，通过全局和局部结构约束指导采样。

Result: 成功在多种动态生物分子系统（如转运蛋白和抗体）中构建原子模型。

Conclusion: CryoBoltz为动态生物分子系统的结构建模提供了一种灵活而强大的方法。

Abstract: Protein structure prediction models are now capable of generating accurate 3D
structural hypotheses from sequence alone. However, they routinely fail to
capture the conformational diversity of dynamic biomolecular complexes, often
requiring heuristic MSA subsampling approaches for generating alternative
states. In parallel, cryo-electron microscopy (cryo-EM) has emerged as a
powerful tool for imaging near-native structural heterogeneity, but is
challenged by arduous pipelines to go from raw experimental data to atomic
models. Here, we bridge the gap between these modalities, combining cryo-EM
density maps with the rich sequence and biophysical priors learned by protein
structure prediction models. Our method, CryoBoltz, guides the sampling
trajectory of a pretrained protein structure prediction model using both global
and local structural constraints derived from density maps, driving predictions
towards conformational states consistent with the experimental data. We
demonstrate that this flexible yet powerful inference-time approach allows us
to build atomic models into heterogeneous cryo-EM maps across a variety of
dynamic biomolecular systems including transporters and antibodies.

</details>


### [23] [NOBLE -- Neural Operator with Biologically-informed Latent Embeddings to Capture Experimental Variability in Biological Neuron Models](https://arxiv.org/abs/2506.04536)
*Luca Ghafourpour,Valentin Duruisseaux,Bahareh Tolooshams,Philip H. Wong,Costas A. Anastassiou,Anima Anandkumar*

Main category: cs.LG

TL;DR: NOBLE是一种神经算子框架，通过学习神经元特征的连续频率调制嵌入到电压响应的映射，解决了传统神经元模型在捕获生物物理复杂性和变异性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 当前神经元模型受限于实验数据的有限性和变异性，难以高效生成具有生物物理复杂性的模型。

Method: NOBLE通过深度学习框架学习神经元特征的嵌入映射，预测神经动态分布，并利用嵌入空间插值生成与实验一致的模型。

Result: NOBLE实现了4200倍的加速，并成功捕获神经元的变异性，生成与实验数据一致的合成神经元。

Conclusion: NOBLE为理解神经元组成、计算及神经AI应用提供了新工具。

Abstract: Characterizing the diverse computational properties of human neurons via
multimodal electrophysiological, transcriptomic, and morphological data
provides the foundation for constructing and validating bio-realistic neuron
models that can advance our understanding of fundamental mechanisms underlying
brain function. However, current modeling approaches remain constrained by the
limited availability and intrinsic variability of experimental neuronal data.
To capture variability, ensembles of deterministic models are often used, but
are difficult to scale as model generation requires repeating computationally
expensive optimization for each neuron. While deep learning is becoming
increasingly relevant in this space, it fails to capture the full biophysical
complexity of neurons, their nonlinear voltage dynamics, and variability. To
address these shortcomings, we introduce NOBLE, a neural operator framework
that learns a mapping from a continuous frequency-modulated embedding of
interpretable neuron features to the somatic voltage response induced by
current injection. Trained on data generated from biophysically realistic
neuron models, NOBLE predicts distributions of neural dynamics accounting for
the intrinsic experimental variability. Unlike conventional bio-realistic
neuron models, interpolating within the embedding space offers models whose
dynamics are consistent with experimentally observed responses. NOBLE is the
first scaled-up deep learning framework validated on real experimental data,
enabling efficient generation of synthetic neurons that exhibit trial-to-trial
variability and achieve a $4200\times$ speedup over numerical solvers. To this
end, NOBLE captures fundamental neural properties, opening the door to a better
understanding of cellular composition and computations, neuromorphic
architectures, large-scale brain circuits, and general neuroAI applications.

</details>


### [24] [Associative Memory and Generative Diffusion in the Zero-noise Limit](https://arxiv.org/abs/2506.05178)
*Joshua Hess,Quaid Morris*

Main category: cs.LG

TL;DR: 研究了生成扩散模型与连续状态联想记忆模型之间的联系，强调Morse-Smale动力系统作为梯度联想记忆模型的通用逼近器，并探讨了噪声水平降低时从生成到记忆的通用过渡特性。


<details>
  <summary>Details</summary>
Motivation: 探索生成扩散模型与联想记忆模型之间的理论联系，揭示噪声水平变化对系统行为的影响。

Method: 利用Morse-Smale动力系统作为通用逼近器，分析梯度联想记忆模型和扩散模型在零噪声极限下的稳定性。

Result: 表明在零噪声极限下，系统具有结构稳定性，且通过分岔序列可以描述学习与生成景观的定性转变。

Conclusion: 研究为理解生成扩散模型与联想记忆模型的关系提供了理论框架，并揭示了噪声水平对系统稳定性的重要性。

Abstract: Connections between generative diffusion and continuous-state associative
memory models are studied. Morse-Smale dynamical systems are emphasized as
universal approximators of gradient-based associative memory models and
diffusion models as white-noise perturbed systems thereof. Universal properties
of associative memory that follow from this description are described and used
to characterize a generic transition from generation to memory as noise levels
diminish. Structural stability inherited by Morse-Smale flows is shown to imply
a notion of stability for diffusions at vanishing noise levels. Applied to one-
and two-parameter families of gradients, this indicates stability at all but
isolated points of associative memory learning landscapes and the learning and
generation landscapes of diffusion models with gradient drift in the zero-noise
limit, at which small sets of generic bifurcations characterize qualitative
transitions between stable systems. Examples illustrating the characterization
of these landscapes by sequences of these bifurcations are given, along with
structural stability criterion for classic and modern Hopfield networks
(equivalently, the attention mechanism).

</details>


<div id='cond-mat.dis-nn'></div>

# cond-mat.dis-nn [[Back]](#toc)

### [25] [Transient dynamics of associative memory models](https://arxiv.org/abs/2506.05303)
*David G. Clark*

Main category: cond-mat.dis-nn

TL;DR: 论文探讨了联想记忆模型（如Hopfield网络）在高阶交互下的“黑障灾难”现象，提出动态视角下即使超过容量限制，模式仍可被短暂高精度检索。


<details>
  <summary>Details</summary>
Motivation: 传统平衡视角认为超过容量限制后网络不可用，但作者认为这是视角局限，希望通过动态分析揭示更多可能性。

Method: 使用二分腔方法和动态平均场方程，结合数值方案分析高阶交互的联想记忆模型。

Result: 发现即使超过容量限制，模式仍可通过不稳定浅盆地短暂检索，且动态恢复曲线展示了非灾难性变化。

Conclusion: 动态视角揭示了传统平衡分析忽略的丰富能量景观结构，为生物神经回路利用瞬态动态提供理论支持，并推动了Hopfield模型的扩展研究。

Abstract: Associative memory models such as the Hopfield network and its dense
generalizations with higher-order interactions exhibit a "blackout
catastrophe"--a discontinuous transition where stable memory states abruptly
vanish when the number of stored patterns exceeds a critical capacity. This
transition is often interpreted as rendering networks unusable beyond capacity
limits. We argue that this interpretation is largely an artifact of the
equilibrium perspective. We derive dynamical mean-field equations using a
bipartite cavity approach for graded-activity dense associative memory models,
with the Hopfield model as a special case, and solve them using a numerical
scheme. We show that patterns can be transiently retrieved with high accuracy
above capacity despite the absence of stable attractors. This occurs because
slow regions persist in the above-capacity energy landscape as shallow,
unstable remnants of below-capacity stable basins. The same transient-retrieval
effect occurs in below-capacity networks initialized outside basins of
attraction. "Transient-recovery curves" provide a concise visual summary of
these effects, revealing graceful, non-catastrophic changes in retrieval
behavior above capacity and allowing us to compare the behavior across
interaction orders. This dynamical perspective reveals rich energy landscape
structure obscured by equilibrium analysis and suggests biological neural
circuits may exploit transient dynamics for memory retrieval. Furthermore, our
approach suggests ways of understanding computational properties of neural
circuits without reference to fixed points, advances the technical repertoire
of numerical mean-field solution methods for recurrent neural networks, and
yields new theoretical results on generalizations of the Hopfield model.

</details>


### [26] [Transient dynamics of associative memory models](https://arxiv.org/abs/2506.05303)
*David G. Clark*

Main category: cond-mat.dis-nn

TL;DR: 论文探讨了联想记忆模型（如Hopfield网络）在超过临界容量时的动态行为，挑战了传统平衡视角下的‘黑屏灾难’解释，并展示了在不稳定状态下仍可高精度检索记忆的模式。


<details>
  <summary>Details</summary>
Motivation: 传统观点认为联想记忆模型在超过容量限制时会因‘黑屏灾难’而失效，但作者认为这是平衡视角的局限，希望通过动态分析揭示更丰富的能量景观结构。

Method: 作者使用二分空腔方法推导了动态平均场方程，并通过数值方案求解，研究了Hopfield模型及其高阶推广的动态行为。

Result: 研究发现，即使在超过容量限制时，记忆模式仍可通过瞬态动力学高精度检索，能量景观中存在浅而不稳定的残留区域。

Conclusion: 动态视角揭示了传统平衡分析忽略的丰富结构，表明生物神经回路可能利用瞬态动力学进行记忆检索，并为理解神经网络的计算特性提供了新方法。

Abstract: Associative memory models such as the Hopfield network and its dense
generalizations with higher-order interactions exhibit a "blackout
catastrophe"--a discontinuous transition where stable memory states abruptly
vanish when the number of stored patterns exceeds a critical capacity. This
transition is often interpreted as rendering networks unusable beyond capacity
limits. We argue that this interpretation is largely an artifact of the
equilibrium perspective. We derive dynamical mean-field equations using a
bipartite cavity approach for graded-activity dense associative memory models,
with the Hopfield model as a special case, and solve them using a numerical
scheme. We show that patterns can be transiently retrieved with high accuracy
above capacity despite the absence of stable attractors. This occurs because
slow regions persist in the above-capacity energy landscape as shallow,
unstable remnants of below-capacity stable basins. The same transient-retrieval
effect occurs in below-capacity networks initialized outside basins of
attraction. "Transient-recovery curves" provide a concise visual summary of
these effects, revealing graceful, non-catastrophic changes in retrieval
behavior above capacity and allowing us to compare the behavior across
interaction orders. This dynamical perspective reveals rich energy landscape
structure obscured by equilibrium analysis and suggests biological neural
circuits may exploit transient dynamics for memory retrieval. Furthermore, our
approach suggests ways of understanding computational properties of neural
circuits without reference to fixed points, advances the technical repertoire
of numerical mean-field solution methods for recurrent neural networks, and
yields new theoretical results on generalizations of the Hopfield model.

</details>


### [27] [Transient dynamics of associative memory models](https://arxiv.org/abs/2506.05303)
*David G. Clark*

Main category: cond-mat.dis-nn

TL;DR: 论文通过动力学视角分析联想记忆模型，发现即使超过容量限制，记忆模式仍可被短暂高精度检索，挑战了传统平衡态视角的结论。


<details>
  <summary>Details</summary>
Motivation: 传统平衡态视角认为联想记忆模型在超过容量限制时会突然失效，但作者认为这是视角局限的产物，希望通过动力学分析揭示更丰富的能量景观结构。

Method: 作者使用二分腔方法和数值方案推导并求解了分级活动密集联想记忆模型的动力学平均场方程，以Hopfield模型为例。

Result: 研究表明，即使在没有稳定吸引子的情况下，记忆模式仍可在超过容量限制时被短暂高精度检索，且能量景观中存在浅层不稳定区域。

Conclusion: 动力学视角揭示了传统平衡态分析忽略的丰富结构，表明生物神经回路可能利用瞬态动力学进行记忆检索，并为神经网络的计算特性提供了新理解。

Abstract: Associative memory models such as the Hopfield network and its dense
generalizations with higher-order interactions exhibit a "blackout
catastrophe"--a discontinuous transition where stable memory states abruptly
vanish when the number of stored patterns exceeds a critical capacity. This
transition is often interpreted as rendering networks unusable beyond capacity
limits. We argue that this interpretation is largely an artifact of the
equilibrium perspective. We derive dynamical mean-field equations using a
bipartite cavity approach for graded-activity dense associative memory models,
with the Hopfield model as a special case, and solve them using a numerical
scheme. We show that patterns can be transiently retrieved with high accuracy
above capacity despite the absence of stable attractors. This occurs because
slow regions persist in the above-capacity energy landscape as shallow,
unstable remnants of below-capacity stable basins. The same transient-retrieval
effect occurs in below-capacity networks initialized outside basins of
attraction. "Transient-recovery curves" provide a concise visual summary of
these effects, revealing graceful, non-catastrophic changes in retrieval
behavior above capacity and allowing us to compare the behavior across
interaction orders. This dynamical perspective reveals rich energy landscape
structure obscured by equilibrium analysis and suggests biological neural
circuits may exploit transient dynamics for memory retrieval. Furthermore, our
approach suggests ways of understanding computational properties of neural
circuits without reference to fixed points, advances the technical repertoire
of numerical mean-field solution methods for recurrent neural networks, and
yields new theoretical results on generalizations of the Hopfield model.

</details>


<div id='q-bio.PE'></div>

# q-bio.PE [[Back]](#toc)

### [28] [A note on metapopulation models](https://arxiv.org/abs/2506.04284)
*Diepreye Ayabina,Hasan Sevil,Adam Kleczkowski,M. Gabriela M. Gomes*

Main category: q-bio.PE

TL;DR: 本文研究了传染病流行病学中个体在斑块内变异的影响，忽略斑块间传播，发现忽略斑块内异质性会低估基本再生数（$\mathcal{R}_{0}$）和防控努力，并提出了一种推断个体易感性分布的方法。


<details>
  <summary>Details</summary>
Motivation: 研究斑块内个体变异对传染病传播的影响，补充传统斑块间迁移模型的不足。

Method: 通过忽略斑块间传播，分析斑块内个体异质性对$\mathcal{R}_{0}$和防控努力的影响，并提出基于斑块分层的个体易感性推断方法。

Result: 忽略斑块内异质性会低估$\mathcal{R}_{0}$和防控需求；提出的推断方法可用于COVID-19等案例。

Conclusion: 斑块内个体异质性对传染病传播有重要影响，需在模型中加以考虑以提高预测和防控效果。

Abstract: Metapopulation models are commonly used in ecology, evolution, and
epidemiology. These models usually entail homogeneity assumptions within
patches and study networks of migration between patches to generate insights
into conservation of species, differentiation of populations, and persistence
of infectious diseases. Here, focusing on infectious disease epidemiology, we
take a complementary approach and study the effects of individual variation
within patches while neglecting any form of disease transmission between
patches. Consistently with previous work on single populations, we show how
metapopulation models that neglect in-patch heterogeneity also underestimate
basic reproduction numbers ($\mathcal{R}_{0}$) and the effort required to
control or eliminate infectious diseases by uniform interventions. We then go
beyond this confirmatory result and introduce a scheme to infer distributions
of individual susceptibility or exposure to infection based on suitable
stratifications of a population into patches. We apply the resulting
metapopulation models to a simple case study of the COVID-19 pandemic.

</details>


### [29] [Viral Hitchhikers and Macroevolution: A Novel Hypothesis on Explosive Speciation](https://arxiv.org/abs/2506.04511)
*Mario E. Lozano,Marcela G. Pilloff*

Main category: q-bio.PE

TL;DR: 该论文提出内源性病毒元素（EVEs）通过水平转移和基因组整合，可能成为适应性辐射和新物种形成的催化剂。


<details>
  <summary>Details</summary>
Motivation: 探讨内源性病毒元素在基因组变化、物种形成和适应性辐射中的作用，尤其是在K-Pg事件期间哺乳动物辐射的背景下。

Method: 理论分析，提出EVEs通过整合宿主基因组、诱导染色体重排和水平转移，影响物种形成。

Result: EVEs及其母病毒可能通过同步影响多个个体，促进生殖隔离和新物种的快速形成。

Conclusion: 内源性病毒可能是爆炸性物种形成的催化剂，尤其是在病毒流行期间。

Abstract: Mobile genetic elements (e.g., endogenous viruses, LINEs, SINEs) can transfer
between genomes, even between species, triggering dramatic genetic changes.
Endogenous viral elements (EVEs) arise when infectious viruses integrate into
the host germline. EVEs integrate at specific sites; their genes or regulatory
regions can be exapted and could induce chromosomal rearrangement. We propose
that EVEs participate in adaptive radiations and that their parent viruses,
through interspecific transfer, could initiate new species formation. By
synchronously affecting multiple individuals, viral outbreaks generate shared
genomic changes that both facilitate reproductive isolation and provide the
simultaneous modifications needed for groups to emerge as founders of new
species. We suggest horizontal viral transfer during the K-Pg accelerated
mammalian radiation linking viral epidemics to macroevolutionary
diversification. This theoretical work proposes endogenous viruses as catalysts
for explosive speciation.

</details>


### [30] [A note on metapopulation models](https://arxiv.org/abs/2506.04284)
*Diepreye Ayabina,Hasan Sevil,Adam Kleczkowski,M. Gabriela M. Gomes*

Main category: q-bio.PE

TL;DR: 本文研究了传染病流行病学中个体在斑块内变异的影响，忽略斑块间传播，发现忽略斑块内异质性会低估基本再生数（R0）和统一干预所需的努力。


<details>
  <summary>Details</summary>
Motivation: 传统元种群模型通常假设斑块内同质性，研究斑块间迁移网络，本文则关注斑块内个体变异对传染病流行病学的影响。

Method: 通过忽略斑块间传播，研究斑块内异质性对R0和干预效果的影响，并提出一种推断个体易感性或暴露分布的方法。

Result: 忽略斑块内异质性会低估R0和干预努力，提出的方法可基于斑块分层推断个体易感性分布。

Conclusion: 斑块内异质性在传染病流行病学中至关重要，忽略它会导致对干预效果的误判。

Abstract: Metapopulation models are commonly used in ecology, evolution, and
epidemiology. These models usually entail homogeneity assumptions within
patches and study networks of migration between patches to generate insights
into conservation of species, differentiation of populations, and persistence
of infectious diseases. Here, focusing on infectious disease epidemiology, we
take a complementary approach and study the effects of individual variation
within patches while neglecting any form of disease transmission between
patches. Consistently with previous work on single populations, we show how
metapopulation models that neglect in-patch heterogeneity also underestimate
basic reproduction numbers ($\mathcal{R}_{0}$) and the effort required to
control or eliminate infectious diseases by uniform interventions. We then go
beyond this confirmatory result and introduce a scheme to infer distributions
of individual susceptibility or exposure to infection based on suitable
stratifications of a population into patches. We apply the resulting
metapopulation models to a simple case study of the COVID-19 pandemic.

</details>


### [31] [Viral Hitchhikers and Macroevolution: A Novel Hypothesis on Explosive Speciation](https://arxiv.org/abs/2506.04511)
*Mario E. Lozano,Marcela G. Pilloff*

Main category: q-bio.PE

TL;DR: 论文提出内源性病毒元素（EVEs）通过水平转移和基因组整合，可能在适应性辐射和新物种形成中起催化作用。


<details>
  <summary>Details</summary>
Motivation: 探讨内源性病毒元素（EVEs）如何通过整合宿主基因组并引发遗传变化，从而促进物种形成和适应性辐射。

Method: 通过理论分析，提出EVEs及其亲本病毒通过水平转移和同步基因组变化，促进生殖隔离和新物种形成。

Result: EVEs可能在适应性辐射中起关键作用，例如K-Pg事件中哺乳动物的快速多样化。

Conclusion: 内源性病毒可能是物种爆发性形成的催化剂，尤其在病毒流行期间。

Abstract: Mobile genetic elements (e.g., endogenous viruses, LINEs, SINEs) can transfer
between genomes, even between species, triggering dramatic genetic changes.
Endogenous viral elements (EVEs) arise when infectious viruses integrate into
the host germline. EVEs integrate at specific sites; their genes or regulatory
regions can be exapted and could induce chromosomal rearrangement. We propose
that EVEs participate in adaptive radiations and that their parent viruses,
through interspecific transfer, could initiate new species formation. By
synchronously affecting multiple individuals, viral outbreaks generate shared
genomic changes that both facilitate reproductive isolation and provide the
simultaneous modifications needed for groups to emerge as founders of new
species. We suggest horizontal viral transfer during the K-Pg accelerated
mammalian radiation linking viral epidemics to macroevolutionary
diversification. This theoretical work proposes endogenous viruses as catalysts
for explosive speciation.

</details>


### [32] [A note on metapopulation models](https://arxiv.org/abs/2506.04284)
*Diepreye Ayabina,Hasan Sevil,Adam Kleczkowski,M. Gabriela M. Gomes*

Main category: q-bio.PE

TL;DR: 论文研究了传染病流行病学中个体在斑块内的异质性对基本再生数（R₀）和防控措施的影响，提出了一种推断个体易感性或暴露分布的方法，并应用于COVID-19的案例研究。


<details>
  <summary>Details</summary>
Motivation: 传统斑块模型假设斑块内同质性，忽略了个体差异对传染病传播的影响。本文旨在研究个体异质性对传染病传播和防控的影响。

Method: 通过忽略斑块间传播，专注于斑块内个体异质性，提出了一种推断个体易感性或暴露分布的方法，并将其应用于COVID-19案例。

Result: 研究表明，忽略斑块内异质性的模型会低估R₀和防控措施所需努力。提出的方法能更准确地推断个体易感性或暴露分布。

Conclusion: 个体异质性在传染病传播中起重要作用，忽略它会导致对传播动态和防控需求的低估。提出的方法为更精确的传染病建模提供了工具。

Abstract: Metapopulation models are commonly used in ecology, evolution, and
epidemiology. These models usually entail homogeneity assumptions within
patches and study networks of migration between patches to generate insights
into conservation of species, differentiation of populations, and persistence
of infectious diseases. Here, focusing on infectious disease epidemiology, we
take a complementary approach and study the effects of individual variation
within patches while neglecting any form of disease transmission between
patches. Consistently with previous work on single populations, we show how
metapopulation models that neglect in-patch heterogeneity also underestimate
basic reproduction numbers ($\mathcal{R}_{0}$) and the effort required to
control or eliminate infectious diseases by uniform interventions. We then go
beyond this confirmatory result and introduce a scheme to infer distributions
of individual susceptibility or exposure to infection based on suitable
stratifications of a population into patches. We apply the resulting
metapopulation models to a simple case study of the COVID-19 pandemic.

</details>


### [33] [Viral Hitchhikers and Macroevolution: A Novel Hypothesis on Explosive Speciation](https://arxiv.org/abs/2506.04511)
*Mario E. Lozano,Marcela G. Pilloff*

Main category: q-bio.PE

TL;DR: 该论文提出内源性病毒元素（EVEs）通过水平转移和基因组整合，可能在适应性辐射和新物种形成中起关键作用。


<details>
  <summary>Details</summary>
Motivation: 探讨内源性病毒元素（EVEs）如何通过基因组整合和水平转移影响物种的适应性辐射和宏进化多样性。

Method: 理论分析，提出EVEs通过病毒爆发和基因组整合同步影响多个个体，促进生殖隔离和新物种形成。

Result: EVEs可能是哺乳动物在K-Pg事件后快速辐射的催化剂，病毒爆发与宏进化多样性相关。

Conclusion: 内源性病毒是爆炸性物种形成的潜在催化剂，其作用值得进一步研究。

Abstract: Mobile genetic elements (e.g., endogenous viruses, LINEs, SINEs) can transfer
between genomes, even between species, triggering dramatic genetic changes.
Endogenous viral elements (EVEs) arise when infectious viruses integrate into
the host germline. EVEs integrate at specific sites; their genes or regulatory
regions can be exapted and could induce chromosomal rearrangement. We propose
that EVEs participate in adaptive radiations and that their parent viruses,
through interspecific transfer, could initiate new species formation. By
synchronously affecting multiple individuals, viral outbreaks generate shared
genomic changes that both facilitate reproductive isolation and provide the
simultaneous modifications needed for groups to emerge as founders of new
species. We suggest horizontal viral transfer during the K-Pg accelerated
mammalian radiation linking viral epidemics to macroevolutionary
diversification. This theoretical work proposes endogenous viruses as catalysts
for explosive speciation.

</details>


<div id='q-bio.NC'></div>

# q-bio.NC [[Back]](#toc)

### [34] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*

Main category: q-bio.NC

TL;DR: GAIN模型采用基于网格的结构，提升生物合理性和模型动态性，结合Izhikevich模型实现高效且生物准确的模拟。


<details>
  <summary>Details</summary>
Motivation: 改进神经网络的生物合理性和动态性，模拟生物神经元间的交互。

Method: 使用网格结构促进神经元与最近邻的交互，结合Izhikevich模型。

Result: 实现了计算高效且生物准确的模拟，适用于神经网络开发和大规模仿真。

Conclusion: GAIN模型通过网格结构和Izhikevich模型提升了动态性和准确性，适用于神经科学领域。

Abstract: While many neural networks focus on layers to process information, the GAIN
model uses a grid-based structure to improve biological plausibility and the
dynamics of the model. The grid structure helps neurons to interact with their
closest neighbors and improve their connections with one another, which is seen
in biological neurons. While also being implemented with the Izhikevich model
this approach allows for a computationally efficient and biologically accurate
simulation that can aid in the development of neural networks, large scale
simulations, and the development in the neuroscience field. This adaptation of
the Izhikevich model can improve the dynamics and accuracy of the model,
allowing for its uses to be specialized but efficient.

</details>


### [35] [Discounting and Drug Seeking in Biological Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.04549)
*Vardhan Palod,Pranav Mahajan,Veeky Baths,Boris S. Gutkin*

Main category: q-bio.NC

TL;DR: 该论文探讨了长期物质使用障碍（SUD）患者的行为冲突机制，提出了一种基于分层强化学习（HRL）的模型，解释了药物奖励如何导致行为选择对负面后果不敏感，并验证了高折扣率与成瘾严重性的关联。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于理解成瘾行为中认知与行为的冲突机制，尤其是药物奖励如何通过神经生物学机制影响决策过程。

Method: 方法包括构建一个分层强化学习（HRL）模型，整合时间折扣效应，模拟药物奖励对行为选择的影响。

Result: 结果显示高折扣率会放大药物寻求行为，与成瘾严重性和冲动性相关，并与实证研究一致。

Conclusion: 结论认为成瘾是一种分层决策障碍，模型为理解成瘾行为提供了新视角。

Abstract: Despite a strong desire to quit, individuals with long-term substance use
disorder (SUD) often struggle to resist drug use, even when aware of its
harmful consequences. This disconnect between knowledge and compulsive behavior
reflects a fundamental cognitive-behavioral conflict in addiction.
Neurobiologically, differential cue-induced activity within striatal
subregions, along with dopamine-mediated connectivity from the ventral to the
dorsal striatum, contributes to compulsive drug-seeking. However, the
functional mechanism linking these findings to behavioral conflict remains
unclear. Another hallmark of addiction is temporal discounting: individuals
with drug dependence exhibit steeper discount rates than non-users. Assuming
the ventral-dorsal striatal organization reflects a gradient from cognitive to
motor representations, addiction can be modeled within a hierarchical
reinforcement learning (HRL) framework. However, integrating discounting into
biologically grounded HRL remains an open challenge. In this work, we build on
a model showing how action choices reinforced with drug rewards become
insensitive to the negative consequences that follow. We address the
integration of discounting by ensuring natural reward values converge across
all levels in the HRL hierarchy, while drug rewards diverge due to their
dopaminergic effects. Our results show that high discounting amplifies
drug-seeking across the hierarchy, linking faster discounting with increased
addiction severity and impulsivity. We demonstrate alignment with empirical
findings on temporal discounting and propose testable predictions, establishing
addiction as a disorder of hierarchical decision-making.

</details>


### [36] [Consciousness via MIPT?](https://arxiv.org/abs/2506.04875)
*Alexander Gorsky*

Main category: q-bio.NC

TL;DR: 论文探讨了测量诱导相变（MIPT）在认知网络生成模型和意识状态形成中的作用，提出了探针-目标模型，并将MIPT解释为可学习性相变。


<details>
  <summary>Details</summary>
Motivation: 研究MIPT在非平衡系统中的现象，并探索其在认知网络和意识形成中的潜在作用。

Method: 提出探针-目标模型，将MIPT解释为认知网络中概念的可学习性相变，并与探针中的同步相变进行比较。

Result: 发现MIPT可能在认知网络的生成模型和意识状态形成中起关键作用。

Conclusion: MIPT作为可学习性相变，可能在认知网络和意识形成中具有重要意义。

Abstract: The measurement-induced phase transition (MIPT) is a recently formulated
phenomenon in out-of-equilibrium systems. The competition between unitary
evolutions and measurement-induced non-unitaries leads to the transition
between the entangled and disentangled phases at some critical measurement
rate. We conjecture that self-organized MIPT plays a key role in the generative
model of cognitive networks and the formation of the state of consciousness in
the "newborn-adult" transition. To this aim, we formulate the probe-target
picture for the brain and suggest that MIPT interpreted as learnability
transition takes place in the mental part of the target where the sites in the
cognitive networks of semantic memory are concepts. Comparison with the
synchronization phase transitions in the probe is made.

</details>


### [37] [TRACE: Contrastive learning for multi-trial time-series data in neuroscience](https://arxiv.org/abs/2506.04906)
*Lisa Schmors,Dominic Gonschorek,Jan Niklas Böhm,Yongrong Qiu,Na Zhou,Dmitry Kobak,Andreas Tolias,Fabian Sinz,Jacob Reimer,Katrin Franke,Sebastian Damrich,Philipp Berens*

Main category: q-bio.NC

TL;DR: TRACE是一种新的对比学习框架，通过平均不同试验子集生成正对，用于神经时间序列数据的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现代神经记录技术（如双光子成像）生成了大量神经元响应的时间序列数据，但现有方法未充分利用多试验数据结构。

Method: TRACE结合对比学习和邻居嵌入思想，直接学习二维嵌入，通过平均不同试验子集生成正对。

Result: TRACE在模拟数据中表现优于其他方法，能够分辨细微响应差异；在活体记录中，其表示捕捉了生物相关的连续变异和细胞类型聚类结构。

Conclusion: TRACE是一种有效的神经时间序列表示学习方法，可用于数据质量控制。

Abstract: Modern neural recording techniques such as two-photon imaging allow to
acquire vast time-series datasets with responses of hundreds or thousands of
neurons. Contrastive learning is a powerful self-supervised framework for
learning representations of complex datasets. Existing applications for neural
time series rely on generic data augmentations and do not exploit the
multi-trial data structure inherent in many neural datasets. Here we present
TRACE, a new contrastive learning framework that averages across different
subsets of trials to generate positive pairs. TRACE allows to directly learn a
two-dimensional embedding, combining ideas from contrastive learning and
neighbor embeddings. We show that TRACE outperforms other methods, resolving
fine response differences in simulated data. Further, using in vivo recordings,
we show that the representations learned by TRACE capture both biologically
relevant continuous variation, cell-type-related cluster structure, and can
assist data quality control.

</details>


### [38] [The Hippocampal Place Field Gradient: An Eigenmode Theory Linking Grid Cell Projections to Multiscale Learning](https://arxiv.org/abs/2506.04943)
*Shujun Zhou,Guozhang Chen*

Main category: q-bio.NC

TL;DR: 论文提出了一种理论框架，解释海马体如何通过网格细胞输入的线性投影形成连续梯度的大小不同的位置场，并揭示了这种多尺度组织的功能意义。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决海马体位置场大小梯度形成的原理，以及如何从离散的网格细胞输入生成连续梯度。

Method: 通过数学分析和模拟，提出网格细胞到位置场的连接权重随频率衰减的理论框架，并验证其在少样本学习中的最优性。

Result: 研究表明，位置场大小梯度平衡了精度和泛化的权衡，并揭示了环境结构对最优位置场大小的影响。

Conclusion: 该理论框架为位置场梯度提供了原理性解释，并为生物和人工智能中的自适应学习提供了可测试的预测。

Abstract: The hippocampus encodes space through a striking gradient of place field
sizes along its dorsal-ventral axis, yet the principles generating this
continuous gradient from discrete grid cell inputs remain debated. We propose a
unified theoretical framework establishing that hippocampal place fields arise
naturally as linear projections of grid cell population activity, interpretable
as eigenmodes. Critically, we demonstrate that a frequency-dependent decay of
these grid-to-place connection weights naturally transforms inputs from
discrete grid modules into a continuous spectrum of place field sizes. This
multiscale organization is functionally significant: we reveal it shapes the
inductive bias of the population code, balancing a fundamental trade-off
between precision and generalization. Mathematical analysis and simulations
demonstrate an optimal place field size for few-shot learning, which scales
with environment structure. Our results offer a principled explanation for the
place field gradient and generate testable predictions, bridging anatomical
connectivity with adaptive learning in both biological and artificial
intelligence.

</details>


### [39] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM是一种新型混合架构，结合了跨注意力模块的单个尖峰标记化和递归状态空间模型（SSM）主干，旨在实现快速、因果的在线神经活动预测，并通过多数据集预训练高效泛化到新任务。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法（如简单循环神经网络）泛化能力不足，以及Transformer方法计算资源需求高、不适合实时或低资源环境的问题。

Method: 提出POSSM架构，结合跨注意力模块的尖峰标记化和SSM主干，支持多数据集预训练。

Result: 在猴子和人类任务中，POSSM的解码精度与最先进的Transformer相当，但推理速度更快（GPU上快达9倍）。

Conclusion: 混合SSM是平衡准确性、推理速度和泛化能力的有前景方法，适用于实时闭环神经解码应用。

Abstract: Real-time decoding of neural activity is central to neuroscience and
neurotechnology applications, from closed-loop experiments to brain-computer
interfaces, where models are subject to strict latency constraints. Traditional
methods, including simple recurrent neural networks, are fast and lightweight
but often struggle to generalize to unseen data. In contrast, recent
Transformer-based approaches leverage large-scale pretraining for strong
generalization performance, but typically have much larger computational
requirements and are not always suitable for low-resource or real-time
settings. To address these shortcomings, we present POSSM, a novel hybrid
architecture that combines individual spike tokenization via a cross-attention
module with a recurrent state-space model (SSM) backbone to enable (1) fast and
causal online prediction on neural activity and (2) efficient generalization to
new sessions, individuals, and tasks through multi-dataset pretraining. We
evaluate POSSM's decoding performance and inference speed on intracortical
decoding of monkey motor tasks, and show that it extends to clinical
applications, namely handwriting and speech decoding in human subjects.
Notably, we demonstrate that pretraining on monkey motor-cortical recordings
improves decoding performance on the human handwriting task, highlighting the
exciting potential for cross-species transfer. In all of these tasks, we find
that POSSM achieves decoding accuracy comparable to state-of-the-art
Transformers, at a fraction of the inference cost (up to 9x faster on GPU).
These results suggest that hybrid SSMs are a promising approach to bridging the
gap between accuracy, inference speed, and generalization when training neural
decoders for real-time, closed-loop applications.

</details>


### [40] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*

Main category: q-bio.NC

TL;DR: GAIN模型采用基于网格的结构提升生物合理性和模型动态性，结合Izhikevich模型实现高效计算和生物准确性。


<details>
  <summary>Details</summary>
Motivation: 改进神经网络的生物合理性和动态性，模拟生物神经元的交互方式。

Method: 使用网格结构和Izhikevich模型，使神经元与邻近神经元交互并优化连接。

Result: 实现了计算高效且生物准确的模拟，适用于神经网络开发和大规模仿真。

Conclusion: GAIN模型通过网格结构和Izhikevich模型提升了动态性和准确性，适用于专业且高效的应用。

Abstract: While many neural networks focus on layers to process information, the GAIN
model uses a grid-based structure to improve biological plausibility and the
dynamics of the model. The grid structure helps neurons to interact with their
closest neighbors and improve their connections with one another, which is seen
in biological neurons. While also being implemented with the Izhikevich model
this approach allows for a computationally efficient and biologically accurate
simulation that can aid in the development of neural networks, large scale
simulations, and the development in the neuroscience field. This adaptation of
the Izhikevich model can improve the dynamics and accuracy of the model,
allowing for its uses to be specialized but efficient.

</details>


### [41] [Discounting and Drug Seeking in Biological Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.04549)
*Vardhan Palod,Pranav Mahajan,Veeky Baths,Boris S. Gutkin*

Main category: q-bio.NC

TL;DR: 该论文探讨了长期物质使用障碍（SUD）患者的行为冲突机制，提出了一种基于分层强化学习（HRL）的模型，解释了高折扣率如何加剧药物寻求行为。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决药物成瘾中认知与行为冲突的神经生物学机制，尤其是腹侧-背侧纹状体的功能梯度与时间折扣的关系。

Method: 通过分层强化学习框架，整合时间折扣效应，模拟药物奖励与自然奖励在决策中的不同作用。

Result: 高折扣率在HRL层次中放大了药物寻求行为，与成瘾严重程度和冲动性相关。

Conclusion: 成瘾可视为一种分层决策障碍，研究为理解其机制提供了新视角并提出了可验证的预测。

Abstract: Despite a strong desire to quit, individuals with long-term substance use
disorder (SUD) often struggle to resist drug use, even when aware of its
harmful consequences. This disconnect between knowledge and compulsive behavior
reflects a fundamental cognitive-behavioral conflict in addiction.
Neurobiologically, differential cue-induced activity within striatal
subregions, along with dopamine-mediated connectivity from the ventral to the
dorsal striatum, contributes to compulsive drug-seeking. However, the
functional mechanism linking these findings to behavioral conflict remains
unclear. Another hallmark of addiction is temporal discounting: individuals
with drug dependence exhibit steeper discount rates than non-users. Assuming
the ventral-dorsal striatal organization reflects a gradient from cognitive to
motor representations, addiction can be modeled within a hierarchical
reinforcement learning (HRL) framework. However, integrating discounting into
biologically grounded HRL remains an open challenge. In this work, we build on
a model showing how action choices reinforced with drug rewards become
insensitive to the negative consequences that follow. We address the
integration of discounting by ensuring natural reward values converge across
all levels in the HRL hierarchy, while drug rewards diverge due to their
dopaminergic effects. Our results show that high discounting amplifies
drug-seeking across the hierarchy, linking faster discounting with increased
addiction severity and impulsivity. We demonstrate alignment with empirical
findings on temporal discounting and propose testable predictions, establishing
addiction as a disorder of hierarchical decision-making.

</details>


### [42] [Consciousness via MIPT?](https://arxiv.org/abs/2506.04875)
*Alexander Gorsky*

Main category: q-bio.NC

TL;DR: 论文探讨了测量诱导相变（MIPT）在认知网络生成模型和意识状态形成中的作用，提出了探针-目标模型，并比较了探针中的同步相变。


<details>
  <summary>Details</summary>
Motivation: 研究MIPT在非平衡系统中的现象及其在认知网络和意识形成中的潜在作用。

Method: 提出探针-目标模型，将MIPT解释为可学习性相变，并比较探针中的同步相变。

Result: MIPT可能在认知网络的语义记忆概念中发生，与探针中的同步相变相关。

Conclusion: MIPT在认知网络和意识形成中可能起关键作用，探针-目标模型为理解这一现象提供了新视角。

Abstract: The measurement-induced phase transition (MIPT) is a recently formulated
phenomenon in out-of-equilibrium systems. The competition between unitary
evolutions and measurement-induced non-unitaries leads to the transition
between the entangled and disentangled phases at some critical measurement
rate. We conjecture that self-organized MIPT plays a key role in the generative
model of cognitive networks and the formation of the state of consciousness in
the "newborn-adult" transition. To this aim, we formulate the probe-target
picture for the brain and suggest that MIPT interpreted as learnability
transition takes place in the mental part of the target where the sites in the
cognitive networks of semantic memory are concepts. Comparison with the
synchronization phase transitions in the probe is made.

</details>


### [43] [TRACE: Contrastive learning for multi-trial time-series data in neuroscience](https://arxiv.org/abs/2506.04906)
*Lisa Schmors,Dominic Gonschorek,Jan Niklas Böhm,Yongrong Qiu,Na Zhou,Dmitry Kobak,Andreas Tolias,Fabian Sinz,Jacob Reimer,Katrin Franke,Sebastian Damrich,Philipp Berens*

Main category: q-bio.NC

TL;DR: TRACE是一种新的对比学习框架，通过平均不同试验子集生成正对，用于神经时间序列数据的表示学习。


<details>
  <summary>Details</summary>
Motivation: 现代神经记录技术（如双光子成像）生成了大量神经元响应的时间序列数据，但现有方法未能充分利用多试验数据结构。

Method: TRACE结合对比学习和邻域嵌入思想，通过平均不同试验子集生成正对，直接学习二维嵌入。

Result: TRACE在模拟数据中表现优于其他方法，能够解析细微响应差异；在活体记录中，其表示捕捉了生物相关的连续变异和细胞类型聚类结构。

Conclusion: TRACE是一种有效的神经时间序列表示学习方法，可用于数据质量控制。

Abstract: Modern neural recording techniques such as two-photon imaging allow to
acquire vast time-series datasets with responses of hundreds or thousands of
neurons. Contrastive learning is a powerful self-supervised framework for
learning representations of complex datasets. Existing applications for neural
time series rely on generic data augmentations and do not exploit the
multi-trial data structure inherent in many neural datasets. Here we present
TRACE, a new contrastive learning framework that averages across different
subsets of trials to generate positive pairs. TRACE allows to directly learn a
two-dimensional embedding, combining ideas from contrastive learning and
neighbor embeddings. We show that TRACE outperforms other methods, resolving
fine response differences in simulated data. Further, using in vivo recordings,
we show that the representations learned by TRACE capture both biologically
relevant continuous variation, cell-type-related cluster structure, and can
assist data quality control.

</details>


### [44] [The Hippocampal Place Field Gradient: An Eigenmode Theory Linking Grid Cell Projections to Multiscale Learning](https://arxiv.org/abs/2506.04943)
*Shujun Zhou,Guozhang Chen*

Main category: q-bio.NC

TL;DR: 海马体通过其背腹轴的显著梯度编码空间，本文提出统一理论框架，证明位置场是网格细胞活动的线性投影，且权重衰减形成连续梯度。


<details>
  <summary>Details</summary>
Motivation: 探讨海马体位置场梯度如何从离散的网格细胞输入生成，并揭示其功能意义。

Method: 提出理论框架，将网格细胞活动投影为位置场，分析权重衰减对梯度的影响，并通过数学分析和模拟验证。

Result: 发现位置场梯度平衡了精度与泛化的权衡，并揭示了少样本学习中的最优位置场大小。

Conclusion: 为位置场梯度提供理论解释，并生成可测试的预测，连接解剖结构与适应性学习。

Abstract: The hippocampus encodes space through a striking gradient of place field
sizes along its dorsal-ventral axis, yet the principles generating this
continuous gradient from discrete grid cell inputs remain debated. We propose a
unified theoretical framework establishing that hippocampal place fields arise
naturally as linear projections of grid cell population activity, interpretable
as eigenmodes. Critically, we demonstrate that a frequency-dependent decay of
these grid-to-place connection weights naturally transforms inputs from
discrete grid modules into a continuous spectrum of place field sizes. This
multiscale organization is functionally significant: we reveal it shapes the
inductive bias of the population code, balancing a fundamental trade-off
between precision and generalization. Mathematical analysis and simulations
demonstrate an optimal place field size for few-shot learning, which scales
with environment structure. Our results offer a principled explanation for the
place field gradient and generate testable predictions, bridging anatomical
connectivity with adaptive learning in both biological and artificial
intelligence.

</details>


### [45] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM是一种新型混合架构，结合了跨注意力模块的个体尖峰标记化和循环状态空间模型（SSM），旨在实现快速、因果的神经活动在线预测，并通过多数据集预训练高效泛化。


<details>
  <summary>Details</summary>
Motivation: 传统方法（如简单循环神经网络）速度快但泛化能力差，而基于Transformer的方法泛化能力强但计算成本高，不适合实时或低资源场景。POSSM旨在弥补这些不足。

Method: POSSM结合跨注意力模块的尖峰标记化和循环SSM主干，支持快速在线预测和多数据集预训练。

Result: 在猴子和人类的运动任务解码中，POSSM的精度与最先进的Transformer相当，但推理速度更快（GPU上快达9倍）。

Conclusion: 混合SSM是平衡精度、推理速度和泛化能力的有效方法，适用于实时闭环神经解码应用。

Abstract: Real-time decoding of neural activity is central to neuroscience and
neurotechnology applications, from closed-loop experiments to brain-computer
interfaces, where models are subject to strict latency constraints. Traditional
methods, including simple recurrent neural networks, are fast and lightweight
but often struggle to generalize to unseen data. In contrast, recent
Transformer-based approaches leverage large-scale pretraining for strong
generalization performance, but typically have much larger computational
requirements and are not always suitable for low-resource or real-time
settings. To address these shortcomings, we present POSSM, a novel hybrid
architecture that combines individual spike tokenization via a cross-attention
module with a recurrent state-space model (SSM) backbone to enable (1) fast and
causal online prediction on neural activity and (2) efficient generalization to
new sessions, individuals, and tasks through multi-dataset pretraining. We
evaluate POSSM's decoding performance and inference speed on intracortical
decoding of monkey motor tasks, and show that it extends to clinical
applications, namely handwriting and speech decoding in human subjects.
Notably, we demonstrate that pretraining on monkey motor-cortical recordings
improves decoding performance on the human handwriting task, highlighting the
exciting potential for cross-species transfer. In all of these tasks, we find
that POSSM achieves decoding accuracy comparable to state-of-the-art
Transformers, at a fraction of the inference cost (up to 9x faster on GPU).
These results suggest that hybrid SSMs are a promising approach to bridging the
gap between accuracy, inference speed, and generalization when training neural
decoders for real-time, closed-loop applications.

</details>


### [46] [The GAIN Model: A Nature-Inspired Neural Network Framework Based on an Adaptation of the Izhikevich Model](https://arxiv.org/abs/2506.04247)
*Gage K. R. Hooper*

Main category: q-bio.NC

TL;DR: GAIN模型采用基于网格的结构，提升生物合理性和模型动态性，结合Izhikevich模型实现高效且生物准确的模拟。


<details>
  <summary>Details</summary>
Motivation: 提升神经网络的生物合理性和动态性，模拟生物神经元的交互方式。

Method: 使用网格结构促进神经元与最近邻的交互，结合Izhikevich模型。

Result: 实现了计算高效且生物准确的模拟，适用于神经网络开发和大规模仿真。

Conclusion: GAIN模型通过网格结构和Izhikevich模型的结合，提升了动态性和准确性，具有高效和专业化应用的潜力。

Abstract: While many neural networks focus on layers to process information, the GAIN
model uses a grid-based structure to improve biological plausibility and the
dynamics of the model. The grid structure helps neurons to interact with their
closest neighbors and improve their connections with one another, which is seen
in biological neurons. While also being implemented with the Izhikevich model
this approach allows for a computationally efficient and biologically accurate
simulation that can aid in the development of neural networks, large scale
simulations, and the development in the neuroscience field. This adaptation of
the Izhikevich model can improve the dynamics and accuracy of the model,
allowing for its uses to be specialized but efficient.

</details>


### [47] [Discounting and Drug Seeking in Biological Hierarchical Reinforcement Learning](https://arxiv.org/abs/2506.04549)
*Vardhan Palod,Pranav Mahajan,Veeky Baths,Boris S. Gutkin*

Main category: q-bio.NC

TL;DR: 该论文探讨了长期物质使用障碍（SUD）患者认知行为冲突的神经生物学机制，提出了一种基于分层强化学习（HRL）的模型，解释了时间折扣如何加剧药物寻求行为。


<details>
  <summary>Details</summary>
Motivation: 研究动机是理解SUD患者明知有害却仍无法控制药物使用的认知行为冲突，并探索其背后的神经生物学机制。

Method: 通过分层强化学习框架，整合时间折扣效应，模拟药物奖励如何导致行为对负面后果不敏感。

Result: 结果表明，高时间折扣率会加剧药物寻求行为，与成瘾严重程度和冲动性相关。

Conclusion: 结论认为成瘾是一种分层决策障碍，模型与实证数据一致，并提出了可测试的预测。

Abstract: Despite a strong desire to quit, individuals with long-term substance use
disorder (SUD) often struggle to resist drug use, even when aware of its
harmful consequences. This disconnect between knowledge and compulsive behavior
reflects a fundamental cognitive-behavioral conflict in addiction.
Neurobiologically, differential cue-induced activity within striatal
subregions, along with dopamine-mediated connectivity from the ventral to the
dorsal striatum, contributes to compulsive drug-seeking. However, the
functional mechanism linking these findings to behavioral conflict remains
unclear. Another hallmark of addiction is temporal discounting: individuals
with drug dependence exhibit steeper discount rates than non-users. Assuming
the ventral-dorsal striatal organization reflects a gradient from cognitive to
motor representations, addiction can be modeled within a hierarchical
reinforcement learning (HRL) framework. However, integrating discounting into
biologically grounded HRL remains an open challenge. In this work, we build on
a model showing how action choices reinforced with drug rewards become
insensitive to the negative consequences that follow. We address the
integration of discounting by ensuring natural reward values converge across
all levels in the HRL hierarchy, while drug rewards diverge due to their
dopaminergic effects. Our results show that high discounting amplifies
drug-seeking across the hierarchy, linking faster discounting with increased
addiction severity and impulsivity. We demonstrate alignment with empirical
findings on temporal discounting and propose testable predictions, establishing
addiction as a disorder of hierarchical decision-making.

</details>


### [48] [Consciousness via MIPT?](https://arxiv.org/abs/2506.04875)
*Alexander Gorsky*

Main category: q-bio.NC

TL;DR: 本文探讨了测量诱导相变（MIPT）在认知网络生成模型和意识状态形成中的作用，提出了脑的探针-目标模型，并将MIPT解释为可学习性相变。


<details>
  <summary>Details</summary>
Motivation: 研究MIPT在非平衡系统中的现象，并探索其在认知网络和意识形成中的潜在作用。

Method: 提出探针-目标模型，将MIPT解释为认知网络中概念的可学习性相变，并与探针中的同步相变进行比较。

Result: MIPT可能在认知网络和意识状态的形成中起关键作用，特别是在“新生儿-成人”过渡阶段。

Conclusion: MIPT作为可学习性相变，可能在认知网络和意识形成中具有重要影响，为理解大脑功能提供了新视角。

Abstract: The measurement-induced phase transition (MIPT) is a recently formulated
phenomenon in out-of-equilibrium systems. The competition between unitary
evolutions and measurement-induced non-unitaries leads to the transition
between the entangled and disentangled phases at some critical measurement
rate. We conjecture that self-organized MIPT plays a key role in the generative
model of cognitive networks and the formation of the state of consciousness in
the "newborn-adult" transition. To this aim, we formulate the probe-target
picture for the brain and suggest that MIPT interpreted as learnability
transition takes place in the mental part of the target where the sites in the
cognitive networks of semantic memory are concepts. Comparison with the
synchronization phase transitions in the probe is made.

</details>


### [49] [TRACE: Contrastive learning for multi-trial time-series data in neuroscience](https://arxiv.org/abs/2506.04906)
*Lisa Schmors,Dominic Gonschorek,Jan Niklas Böhm,Yongrong Qiu,Na Zhou,Dmitry Kobak,Andreas Tolias,Fabian Sinz,Jacob Reimer,Katrin Franke,Sebastian Damrich,Philipp Berens*

Main category: q-bio.NC

TL;DR: TRACE是一种新的对比学习框架，通过平均不同试验子集生成正对，用于分析神经时间序列数据，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现代神经记录技术（如双光子成像）生成大量时间序列数据，但现有对比学习方法未充分利用多试验数据结构。

Method: TRACE通过平均不同试验子集生成正对，结合对比学习和邻居嵌入思想，直接学习二维嵌入。

Result: TRACE在模拟数据中能分辨细微响应差异，在体内记录中捕捉生物相关连续变异和细胞类型聚类结构。

Conclusion: TRACE是一种有效的神经数据分析工具，可辅助质量控制并揭示生物相关特征。

Abstract: Modern neural recording techniques such as two-photon imaging allow to
acquire vast time-series datasets with responses of hundreds or thousands of
neurons. Contrastive learning is a powerful self-supervised framework for
learning representations of complex datasets. Existing applications for neural
time series rely on generic data augmentations and do not exploit the
multi-trial data structure inherent in many neural datasets. Here we present
TRACE, a new contrastive learning framework that averages across different
subsets of trials to generate positive pairs. TRACE allows to directly learn a
two-dimensional embedding, combining ideas from contrastive learning and
neighbor embeddings. We show that TRACE outperforms other methods, resolving
fine response differences in simulated data. Further, using in vivo recordings,
we show that the representations learned by TRACE capture both biologically
relevant continuous variation, cell-type-related cluster structure, and can
assist data quality control.

</details>


### [50] [The Hippocampal Place Field Gradient: An Eigenmode Theory Linking Grid Cell Projections to Multiscale Learning](https://arxiv.org/abs/2506.04943)
*Shujun Zhou,Guozhang Chen*

Main category: q-bio.NC

TL;DR: 海马体通过其背腹轴上的位置场大小梯度编码空间，本文提出统一理论框架，解释该梯度如何由离散网格细胞输入生成。


<details>
  <summary>Details</summary>
Motivation: 探讨海马体位置场梯度形成的原理，解决离散网格细胞输入如何转化为连续梯度的争议。

Method: 提出理论框架，将位置场视为网格细胞活动的线性投影（特征模式），并通过频率依赖的连接权重衰减实现梯度。

Result: 揭示了多尺度组织的功能意义，优化了学习中的精度与泛化权衡，并提出了可测试的预测。

Conclusion: 为位置场梯度提供了理论解释，连接了解剖结构与适应性学习，对生物和人工智能均有启示。

Abstract: The hippocampus encodes space through a striking gradient of place field
sizes along its dorsal-ventral axis, yet the principles generating this
continuous gradient from discrete grid cell inputs remain debated. We propose a
unified theoretical framework establishing that hippocampal place fields arise
naturally as linear projections of grid cell population activity, interpretable
as eigenmodes. Critically, we demonstrate that a frequency-dependent decay of
these grid-to-place connection weights naturally transforms inputs from
discrete grid modules into a continuous spectrum of place field sizes. This
multiscale organization is functionally significant: we reveal it shapes the
inductive bias of the population code, balancing a fundamental trade-off
between precision and generalization. Mathematical analysis and simulations
demonstrate an optimal place field size for few-shot learning, which scales
with environment structure. Our results offer a principled explanation for the
place field gradient and generate testable predictions, bridging anatomical
connectivity with adaptive learning in both biological and artificial
intelligence.

</details>


### [51] [Generalizable, real-time neural decoding with hybrid state-space models](https://arxiv.org/abs/2506.05320)
*Avery Hee-Woon Ryoo,Nanda H. Krishna,Ximeng Mao,Mehdi Azabou,Eva L. Dyer,Matthew G. Perich,Guillaume Lajoie*

Main category: q-bio.NC

TL;DR: POSSM是一种新型混合架构，结合了交叉注意力模块和循环状态空间模型，用于实时神经活动解码，兼顾速度和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法泛化能力不足和Transformer计算资源需求高的问题，适用于实时和低资源场景。

Method: 采用交叉注意力模块进行单个尖峰标记化，结合循环状态空间模型，支持多数据集预训练。

Result: 在猴子和人类任务中，POSSM解码精度与最先进Transformer相当，推理速度提升高达9倍。

Conclusion: 混合SSM是平衡精度、速度和泛化的有前景方法，适用于实时闭环应用。

Abstract: Real-time decoding of neural activity is central to neuroscience and
neurotechnology applications, from closed-loop experiments to brain-computer
interfaces, where models are subject to strict latency constraints. Traditional
methods, including simple recurrent neural networks, are fast and lightweight
but often struggle to generalize to unseen data. In contrast, recent
Transformer-based approaches leverage large-scale pretraining for strong
generalization performance, but typically have much larger computational
requirements and are not always suitable for low-resource or real-time
settings. To address these shortcomings, we present POSSM, a novel hybrid
architecture that combines individual spike tokenization via a cross-attention
module with a recurrent state-space model (SSM) backbone to enable (1) fast and
causal online prediction on neural activity and (2) efficient generalization to
new sessions, individuals, and tasks through multi-dataset pretraining. We
evaluate POSSM's decoding performance and inference speed on intracortical
decoding of monkey motor tasks, and show that it extends to clinical
applications, namely handwriting and speech decoding in human subjects.
Notably, we demonstrate that pretraining on monkey motor-cortical recordings
improves decoding performance on the human handwriting task, highlighting the
exciting potential for cross-species transfer. In all of these tasks, we find
that POSSM achieves decoding accuracy comparable to state-of-the-art
Transformers, at a fraction of the inference cost (up to 9x faster on GPU).
These results suggest that hybrid SSMs are a promising approach to bridging the
gap between accuracy, inference speed, and generalization when training neural
decoders for real-time, closed-loop applications.

</details>


<div id='q-bio.CB'></div>

# q-bio.CB [[Back]](#toc)

### [52] [Bacterial Chemotaxis in a Traveling Wave Attractant Environment](https://arxiv.org/abs/2506.04702)
*Shobhan Dev Mandal,Sakuntala Chatterjee*

Main category: q-bio.CB

TL;DR: 研究了单细胞大肠杆菌在时空变化的吸引物环境中的趋化行为，发现趋化漂移速度随吸引物波速呈现非单调变化，包括符号反转和多峰值现象。


<details>
  <summary>Details</summary>
Motivation: 探索单细胞在动态环境中的趋化行为，揭示其漂移速度与吸引物波速的复杂关系。

Method: 通过模拟时空变化的吸引物浓度（正弦波形式），测量不同波速下的趋化漂移速度。

Result: 趋化漂移速度随波速变化呈现负峰值、符号反转、正峰值直至归零的复杂行为。

Conclusion: 细胞在波速匹配时能“骑乘”波而感知不到梯度，导致漂移速度的反直觉行为。

Abstract: We study single cell E.coli chemotaxis in a spatio-temporally varying
attractant environment. Modeling the attractant concentration in the form of a
traveling sine wave, we measure in our simulations, the chemotactic drift
velocity of the cell for different propagation speed of the attractant wave. We
find a highly non-trivial dependence where the chemotactic drift velocity
changes sign, and also shows multiple peaks. For slowly moving attractant wave,
drift velocity is negative, i.e. the drift motion is directed opposite to wave
propagation. As the wave speed increases, drift velocity shows a negative peak,
then changes sign, reaches a positive peak and finally becomes zero when the
wave moves too fast for the cell to respond. We explain this rich behavior from
the difference in attractant gradient perceived by the cell during its run
along the propagation direction and opposite to it. In particular, when the
cell moves in the same direction as the wave, the relative velocity of the cell
with respect to the wave becomes zero when the wave speed matches the run
speed. In this limit, the cell is able to ride the wave and experiences no
concentration gradient during these runs. On the contrary, for runs in the
opposite direction, no such effect is present and the effective gradient
increases monotonically with the wave speed. We show, using detailed
quantitative measurements, how this difference gives rise to the
counter-intuitive behavior of chemotactic drift velocity described above.

</details>


### [53] [Bacterial Chemotaxis in a Traveling Wave Attractant Environment](https://arxiv.org/abs/2506.04702)
*Shobhan Dev Mandal,Sakuntala Chatterjee*

Main category: q-bio.CB

TL;DR: 研究单细胞E.coli在时空变化的吸引物环境中的趋化行为，发现趋化漂移速度与吸引物波速呈非单调关系，包括符号变化和多峰现象。


<details>
  <summary>Details</summary>
Motivation: 探索单细胞在动态环境中的趋化行为，理解趋化漂移速度与吸引物波速的复杂关系。

Method: 通过模拟吸引物浓度以正弦波形式传播的环境，测量不同波速下的趋化漂移速度。

Result: 趋化漂移速度随波速变化呈现负峰、符号反转、正峰及最终归零的复杂行为。

Conclusion: 细胞在波传播方向与反向运行时感知的梯度差异导致了这种反直觉的趋化行为。

Abstract: We study single cell E.coli chemotaxis in a spatio-temporally varying
attractant environment. Modeling the attractant concentration in the form of a
traveling sine wave, we measure in our simulations, the chemotactic drift
velocity of the cell for different propagation speed of the attractant wave. We
find a highly non-trivial dependence where the chemotactic drift velocity
changes sign, and also shows multiple peaks. For slowly moving attractant wave,
drift velocity is negative, i.e. the drift motion is directed opposite to wave
propagation. As the wave speed increases, drift velocity shows a negative peak,
then changes sign, reaches a positive peak and finally becomes zero when the
wave moves too fast for the cell to respond. We explain this rich behavior from
the difference in attractant gradient perceived by the cell during its run
along the propagation direction and opposite to it. In particular, when the
cell moves in the same direction as the wave, the relative velocity of the cell
with respect to the wave becomes zero when the wave speed matches the run
speed. In this limit, the cell is able to ride the wave and experiences no
concentration gradient during these runs. On the contrary, for runs in the
opposite direction, no such effect is present and the effective gradient
increases monotonically with the wave speed. We show, using detailed
quantitative measurements, how this difference gives rise to the
counter-intuitive behavior of chemotactic drift velocity described above.

</details>


### [54] [Bacterial Chemotaxis in a Traveling Wave Attractant Environment](https://arxiv.org/abs/2506.04702)
*Shobhan Dev Mandal,Sakuntala Chatterjee*

Main category: q-bio.CB

TL;DR: 研究了单细胞大肠杆菌在时空变化的吸引物环境中的趋化行为，发现趋化漂移速度与吸引物波速呈现非单调依赖关系，包括符号变化和多峰值现象。


<details>
  <summary>Details</summary>
Motivation: 探索单细胞大肠杆菌在动态吸引物环境中的趋化行为，揭示其漂移速度与吸引物波速的复杂关系。

Method: 通过模拟吸引物浓度以正弦波形式传播的环境，测量不同波速下细胞的趋化漂移速度。

Result: 趋化漂移速度随波速变化呈现负峰值、符号反转、正峰值，最终趋近于零。

Conclusion: 细胞在波传播方向与反向运行时感知的吸引物梯度差异导致了这一复杂行为。

Abstract: We study single cell E.coli chemotaxis in a spatio-temporally varying
attractant environment. Modeling the attractant concentration in the form of a
traveling sine wave, we measure in our simulations, the chemotactic drift
velocity of the cell for different propagation speed of the attractant wave. We
find a highly non-trivial dependence where the chemotactic drift velocity
changes sign, and also shows multiple peaks. For slowly moving attractant wave,
drift velocity is negative, i.e. the drift motion is directed opposite to wave
propagation. As the wave speed increases, drift velocity shows a negative peak,
then changes sign, reaches a positive peak and finally becomes zero when the
wave moves too fast for the cell to respond. We explain this rich behavior from
the difference in attractant gradient perceived by the cell during its run
along the propagation direction and opposite to it. In particular, when the
cell moves in the same direction as the wave, the relative velocity of the cell
with respect to the wave becomes zero when the wave speed matches the run
speed. In this limit, the cell is able to ride the wave and experiences no
concentration gradient during these runs. On the contrary, for runs in the
opposite direction, no such effect is present and the effective gradient
increases monotonically with the wave speed. We show, using detailed
quantitative measurements, how this difference gives rise to the
counter-intuitive behavior of chemotactic drift velocity described above.

</details>


<div id='physics.soc-ph'></div>

# physics.soc-ph [[Back]](#toc)

### [55] [Direct reciprocity in asynchronous interactions](https://arxiv.org/abs/2506.04264)
*Ketian Sun,Qi Su,Long Wang*

Main category: physics.soc-ph

TL;DR: 研究探讨了异步互动中直接互惠对合作的影响，发现异步性通常抑制合作，但在特定环境反馈条件下可支持稳定合作策略。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的互动多为异步，而现有研究多假设同步决策，因此需探索异步互动中的合作机制。

Method: 提出基于异步随机游戏的框架，结合异步决策和环境动态反馈，分析合作纳什均衡的条件。

Result: 异步互动通常抑制合作，但在环境反馈达到临界阈值时，任何合作策略均可形成纳什均衡。

Conclusion: 互动顺序在长期进化中至关重要，环境反馈是稳定异步合作的关键因素。

Abstract: Cooperation is vital for the survival of living systems but is challenging
due to the costs borne by altruistic individuals. Direct reciprocity, where
actions are based on past encounters, is a key mechanism fostering cooperation.
However, most studies assume synchronous decision-making, whereas real-world
interactions are often asynchronous, with individuals acting in sequence. This
asynchrony can undermine standard cooperative strategies like Tit-for-Tat and
Win-Stay Lose-Shift. To better understand cooperation in real-world contexts,
it is crucial to explore the theory of direct reciprocity in asynchronous
interactions. To address this, we introduce a framework based on asynchronous
stochastic games, incorporating asynchronous decisions and dynamic
environmental feedback. We analytically derive the conditions under which
strategies form cooperative Nash equilibria. Our results demonstrate that the
order of interactions can significantly alter outcomes: interaction asynchrony
generally inhibits cooperation, except under specific conditions where
environmental feedback effectively mitigates its negative impact. When
environmental feedback is incorporated, a variety of stable reciprocal
strategies can be sustained. Notably, above a critical environmental threshold,
any cooperative strategy can form a Nash equilibrium. Overall, our work
underscores the importance of interaction order in long-term evolutionary
processes and highlights the pivotal role of environmental feedback in
stabilizing cooperation in asynchronous interactions.

</details>


### [56] [Direct reciprocity in asynchronous interactions](https://arxiv.org/abs/2506.04264)
*Ketian Sun,Qi Su,Long Wang*

Main category: physics.soc-ph

TL;DR: 研究探讨了异步交互中直接互惠对合作的影响，发现交互顺序和环境反馈是关键因素。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的交互通常是异步的，而现有研究多假设同步决策，因此需要探索异步交互下的合作机制。

Method: 提出基于异步随机游戏的框架，分析策略在异步交互中形成合作纳什均衡的条件。

Result: 交互异步性通常抑制合作，但在特定环境反馈条件下可稳定合作策略。

Conclusion: 交互顺序和环境反馈在异步合作中起关键作用，环境反馈超过阈值时，任何合作策略均可形成纳什均衡。

Abstract: Cooperation is vital for the survival of living systems but is challenging
due to the costs borne by altruistic individuals. Direct reciprocity, where
actions are based on past encounters, is a key mechanism fostering cooperation.
However, most studies assume synchronous decision-making, whereas real-world
interactions are often asynchronous, with individuals acting in sequence. This
asynchrony can undermine standard cooperative strategies like Tit-for-Tat and
Win-Stay Lose-Shift. To better understand cooperation in real-world contexts,
it is crucial to explore the theory of direct reciprocity in asynchronous
interactions. To address this, we introduce a framework based on asynchronous
stochastic games, incorporating asynchronous decisions and dynamic
environmental feedback. We analytically derive the conditions under which
strategies form cooperative Nash equilibria. Our results demonstrate that the
order of interactions can significantly alter outcomes: interaction asynchrony
generally inhibits cooperation, except under specific conditions where
environmental feedback effectively mitigates its negative impact. When
environmental feedback is incorporated, a variety of stable reciprocal
strategies can be sustained. Notably, above a critical environmental threshold,
any cooperative strategy can form a Nash equilibrium. Overall, our work
underscores the importance of interaction order in long-term evolutionary
processes and highlights the pivotal role of environmental feedback in
stabilizing cooperation in asynchronous interactions.

</details>


### [57] [Direct reciprocity in asynchronous interactions](https://arxiv.org/abs/2506.04264)
*Ketian Sun,Qi Su,Long Wang*

Main category: physics.soc-ph

TL;DR: 研究探讨了异步互动中直接互惠对合作的影响，发现互动顺序和环境反馈是关键因素。


<details>
  <summary>Details</summary>
Motivation: 现实中的互动多为异步，而传统合作策略（如以牙还牙）在异步环境中效果不佳，需研究异步互动的合作机制。

Method: 提出基于异步随机游戏的框架，结合异步决策和环境反馈，分析合作策略的纳什均衡条件。

Result: 异步互动通常抑制合作，但在特定环境反馈下可稳定合作策略；超过临界环境阈值时，任何合作策略均可形成纳什均衡。

Conclusion: 互动顺序在长期进化中至关重要，环境反馈是稳定异步合作的关键。

Abstract: Cooperation is vital for the survival of living systems but is challenging
due to the costs borne by altruistic individuals. Direct reciprocity, where
actions are based on past encounters, is a key mechanism fostering cooperation.
However, most studies assume synchronous decision-making, whereas real-world
interactions are often asynchronous, with individuals acting in sequence. This
asynchrony can undermine standard cooperative strategies like Tit-for-Tat and
Win-Stay Lose-Shift. To better understand cooperation in real-world contexts,
it is crucial to explore the theory of direct reciprocity in asynchronous
interactions. To address this, we introduce a framework based on asynchronous
stochastic games, incorporating asynchronous decisions and dynamic
environmental feedback. We analytically derive the conditions under which
strategies form cooperative Nash equilibria. Our results demonstrate that the
order of interactions can significantly alter outcomes: interaction asynchrony
generally inhibits cooperation, except under specific conditions where
environmental feedback effectively mitigates its negative impact. When
environmental feedback is incorporated, a variety of stable reciprocal
strategies can be sustained. Notably, above a critical environmental threshold,
any cooperative strategy can form a Nash equilibrium. Overall, our work
underscores the importance of interaction order in long-term evolutionary
processes and highlights the pivotal role of environmental feedback in
stabilizing cooperation in asynchronous interactions.

</details>


<div id='q-bio.TO'></div>

# q-bio.TO [[Back]](#toc)

### [58] [Assessing parameter identifiability of a hemodynamics PDE model using spectral surrogates and dimension reduction](https://arxiv.org/abs/2506.04538)
*Mitchel J. Colebank*

Main category: q-bio.TO

TL;DR: 该研究提出了一种利用多项式混沌展开（PCEs）进行多输出全局敏感性分析和参数可识别性评估的创新方法，解决了生物医学模拟器中参数排名和固定的启发式问题。


<details>
  <summary>Details</summary>
Motivation: 生物医学模拟器的计算逆问题通常面临数据有限和参数维度高的挑战，尤其是在构建医学数字孪生时，需要高效的参数敏感性分析和可识别性评估。

Method: 采用多项式混沌展开（PCEs）进行多输出全局敏感性分析，并结合降维技术量化一维肺血流动力学模型的时间序列敏感性；利用PCEs构建轮廓似然置信区间，评估参数可识别性。

Result: 研究展示了实验设计变化如何改善参数可识别性，并验证了PCEs在偏微分方程问题中实现轮廓似然分析的可行性。

Conclusion: 该工作为参数可识别性分析提供了新方法，并利用PCEs为偏微分方程问题中的轮廓似然分析提供了通用仿真策略。

Abstract: Computational inverse problems for biomedical simulators suffer from limited
data and relatively high parameter dimensionality. This often requires
sensitivity analysis, where parameters of the model are ranked based on their
influence on the specific quantities of interest. This is especially important
for simulators used to build medical digital twins, as the amount of data is
typically limited. For expensive models, such as blood flow models, emulation
is employed to expedite the simulation time. Parameter ranking and fixing using
sensitivity analysis are often heuristic, though, and vary with the specific
application or simulator used. The present study provides an innovative
solution to this problem by leveraging polynomial chaos expansions (PCEs) for
both multioutput global sensitivity analysis and formal parameter
identifiability. For the former, we use dimension reduction to efficiently
quantify time-series sensitivity of a one-dimensional pulmonary hemodynamics
model. We consider both Windkessel and structured tree boundary conditions. We
then use PCEs to construct profile-likelihood confidence intervals to formally
assess parameter identifiability, and show how changes in experimental design
improve identifiability. Our work presents a novel approach to determining
parameter identifiability and leverages a common emulation strategy for
enabling profile-likelihood analysis in problems governed by partial
differential equations.

</details>


### [59] [Assessing parameter identifiability of a hemodynamics PDE model using spectral surrogates and dimension reduction](https://arxiv.org/abs/2506.04538)
*Mitchel J. Colebank*

Main category: q-bio.TO

TL;DR: 论文提出了一种利用多项式混沌展开（PCEs）进行多输出全局敏感性分析和参数可识别性评估的创新方法，解决了生物医学模拟器中计算逆问题的高维参数和有限数据挑战。


<details>
  <summary>Details</summary>
Motivation: 生物医学模拟器在解决计算逆问题时面临数据有限和参数维度高的挑战，尤其是用于构建医学数字孪生时。传统的敏感性分析方法通常是启发式的，且因应用或模拟器不同而变化。

Method: 采用多项式混沌展开（PCEs）进行多输出全局敏感性分析，并结合降维技术量化一维肺血流动力学模型的时间序列敏感性。同时，利用PCEs构建轮廓似然置信区间，评估参数可识别性，并探讨实验设计改进对可识别性的影响。

Result: 研究表明，PCEs方法能够高效量化敏感性，并通过轮廓似然分析在偏微分方程问题中评估参数可识别性。实验设计的优化进一步提升了参数可识别性。

Conclusion: 该研究为生物医学模拟器中的参数敏感性和可识别性问题提供了一种新颖且通用的解决方案，尤其适用于高维参数和有限数据的场景。

Abstract: Computational inverse problems for biomedical simulators suffer from limited
data and relatively high parameter dimensionality. This often requires
sensitivity analysis, where parameters of the model are ranked based on their
influence on the specific quantities of interest. This is especially important
for simulators used to build medical digital twins, as the amount of data is
typically limited. For expensive models, such as blood flow models, emulation
is employed to expedite the simulation time. Parameter ranking and fixing using
sensitivity analysis are often heuristic, though, and vary with the specific
application or simulator used. The present study provides an innovative
solution to this problem by leveraging polynomial chaos expansions (PCEs) for
both multioutput global sensitivity analysis and formal parameter
identifiability. For the former, we use dimension reduction to efficiently
quantify time-series sensitivity of a one-dimensional pulmonary hemodynamics
model. We consider both Windkessel and structured tree boundary conditions. We
then use PCEs to construct profile-likelihood confidence intervals to formally
assess parameter identifiability, and show how changes in experimental design
improve identifiability. Our work presents a novel approach to determining
parameter identifiability and leverages a common emulation strategy for
enabling profile-likelihood analysis in problems governed by partial
differential equations.

</details>


### [60] [Assessing parameter identifiability of a hemodynamics PDE model using spectral surrogates and dimension reduction](https://arxiv.org/abs/2506.04538)
*Mitchel J. Colebank*

Main category: q-bio.TO

TL;DR: 该论文提出了一种利用多项式混沌展开（PCE）解决生物医学模拟器中计算逆问题的方法，用于多输出全局敏感性分析和参数可识别性评估。


<details>
  <summary>Details</summary>
Motivation: 生物医学模拟器在处理计算逆问题时面临数据有限和参数维度高的挑战，尤其在构建医疗数字孪生时，需要高效且准确的参数敏感性分析和可识别性评估。

Method: 采用多项式混沌展开（PCE）进行多输出全局敏感性分析，并结合降维技术量化一维肺血流动力学模型的时间序列敏感性。同时，利用PCE构建轮廓似然置信区间，评估参数可识别性。

Result: 研究展示了PCE在敏感性分析和参数可识别性评估中的有效性，并通过实验设计改进参数可识别性。

Conclusion: 该研究为偏微分方程问题提供了一种新颖的参数可识别性分析方法，并利用通用仿真策略支持轮廓似然分析。

Abstract: Computational inverse problems for biomedical simulators suffer from limited
data and relatively high parameter dimensionality. This often requires
sensitivity analysis, where parameters of the model are ranked based on their
influence on the specific quantities of interest. This is especially important
for simulators used to build medical digital twins, as the amount of data is
typically limited. For expensive models, such as blood flow models, emulation
is employed to expedite the simulation time. Parameter ranking and fixing using
sensitivity analysis are often heuristic, though, and vary with the specific
application or simulator used. The present study provides an innovative
solution to this problem by leveraging polynomial chaos expansions (PCEs) for
both multioutput global sensitivity analysis and formal parameter
identifiability. For the former, we use dimension reduction to efficiently
quantify time-series sensitivity of a one-dimensional pulmonary hemodynamics
model. We consider both Windkessel and structured tree boundary conditions. We
then use PCEs to construct profile-likelihood confidence intervals to formally
assess parameter identifiability, and show how changes in experimental design
improve identifiability. Our work presents a novel approach to determining
parameter identifiability and leverages a common emulation strategy for
enabling profile-likelihood analysis in problems governed by partial
differential equations.

</details>


<div id='cs.CV'></div>

# cs.CV [[Back]](#toc)

### [61] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
*Matthew W. Shinkle,Mark D. Lescroart*

Main category: cs.CV

TL;DR: 论文提出了一种基于激活最大化技术的方法，用于解释DNN编码模型对人类大脑视觉系统的响应，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管DNN编码模型能准确预测大脑对视觉刺激的响应，但缺乏对驱动这些响应的具体特征的理解。

Method: 使用预训练的Inception V3网络提取并自适应下采样激活，通过线性回归预测fMRI响应，并应用激活最大化生成优化图像。

Result: 生成的图像与已知选择性特征相符，并能可靠驱动目标脑区活动。

Conclusion: 激活最大化可成功应用于DNN编码模型，为人类视觉系统的响应特征提供了灵活的表征和调控方法。

Abstract: Deep neural networks (DNNs) trained on visual tasks develop feature
representations that resemble those in the human visual system. Although
DNN-based encoding models can accurately predict brain responses to visual
stimuli, they offer limited insight into the specific features driving these
responses. Here, we demonstrate that activation maximization -- a technique
designed to interpret vision DNNs -- can be applied to DNN-based encoding
models of the human brain. We extract and adaptively downsample activations
from multiple layers of a pretrained Inception V3 network, then use linear
regression to predict fMRI responses. This yields a full image-computable model
of brain responses. Next, we apply activation maximization to generate images
optimized for predicted responses in individual cortical voxels. We find that
these images contain visual characteristics that qualitatively correspond with
known selectivity and enable exploration of selectivity across the visual
cortex. We further extend our method to whole regions of interest (ROIs) of the
brain and validate its efficacy by presenting these images to human
participants in an fMRI study. We find that the generated images reliably drive
activity in targeted regions across both low- and high-level visual areas and
across subjects. These results demonstrate that activation maximization can be
successfully applied to DNN-based encoding models. By addressing key
limitations of alternative approaches that require natively generative models,
our approach enables flexible characterization and modulation of responses
across the human visual system.

</details>


### [62] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
*Matthew W. Shinkle,Mark D. Lescroart*

Main category: cs.CV

TL;DR: 论文提出了一种方法，将激活最大化技术应用于基于DNN的脑编码模型，以生成能够驱动特定脑区反应的图像，并验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管DNN编码模型能预测大脑对视觉刺激的反应，但无法揭示驱动这些反应的具体特征。本文旨在通过激活最大化技术解决这一局限。

Method: 从预训练的Inception V3网络中提取并自适应下采样激活，使用线性回归预测fMRI响应，再通过激活最大化生成优化图像。

Result: 生成的图像能可靠地驱动目标脑区活动，且视觉特征与已知选择性一致。

Conclusion: 激活最大化技术可成功应用于DNN编码模型，为人类视觉系统的响应特征提供了灵活的表征和调控手段。

Abstract: Deep neural networks (DNNs) trained on visual tasks develop feature
representations that resemble those in the human visual system. Although
DNN-based encoding models can accurately predict brain responses to visual
stimuli, they offer limited insight into the specific features driving these
responses. Here, we demonstrate that activation maximization -- a technique
designed to interpret vision DNNs -- can be applied to DNN-based encoding
models of the human brain. We extract and adaptively downsample activations
from multiple layers of a pretrained Inception V3 network, then use linear
regression to predict fMRI responses. This yields a full image-computable model
of brain responses. Next, we apply activation maximization to generate images
optimized for predicted responses in individual cortical voxels. We find that
these images contain visual characteristics that qualitatively correspond with
known selectivity and enable exploration of selectivity across the visual
cortex. We further extend our method to whole regions of interest (ROIs) of the
brain and validate its efficacy by presenting these images to human
participants in an fMRI study. We find that the generated images reliably drive
activity in targeted regions across both low- and high-level visual areas and
across subjects. These results demonstrate that activation maximization can be
successfully applied to DNN-based encoding models. By addressing key
limitations of alternative approaches that require natively generative models,
our approach enables flexible characterization and modulation of responses
across the human visual system.

</details>


### [63] [Visualizing and Controlling Cortical Responses Using Voxel-Weighted Activation Maximization](https://arxiv.org/abs/2506.04379)
*Matthew W. Shinkle,Mark D. Lescroart*

Main category: cs.CV

TL;DR: 论文提出了一种方法，通过激活最大化技术解释DNN编码模型，生成能预测大脑反应的图像，并验证其在视觉系统中的有效性。


<details>
  <summary>Details</summary>
Motivation: 尽管DNN编码模型能预测大脑对视觉刺激的反应，但缺乏对驱动这些反应的具体特征的理解。

Method: 使用预训练的Inception V3网络提取并降采样激活，通过线性回归预测fMRI反应，并应用激活最大化生成优化图像。

Result: 生成的图像能可靠地驱动目标区域的神经活动，验证了方法的有效性。

Conclusion: 激活最大化技术可成功应用于DNN编码模型，为研究人类视觉系统提供了灵活的工具。

Abstract: Deep neural networks (DNNs) trained on visual tasks develop feature
representations that resemble those in the human visual system. Although
DNN-based encoding models can accurately predict brain responses to visual
stimuli, they offer limited insight into the specific features driving these
responses. Here, we demonstrate that activation maximization -- a technique
designed to interpret vision DNNs -- can be applied to DNN-based encoding
models of the human brain. We extract and adaptively downsample activations
from multiple layers of a pretrained Inception V3 network, then use linear
regression to predict fMRI responses. This yields a full image-computable model
of brain responses. Next, we apply activation maximization to generate images
optimized for predicted responses in individual cortical voxels. We find that
these images contain visual characteristics that qualitatively correspond with
known selectivity and enable exploration of selectivity across the visual
cortex. We further extend our method to whole regions of interest (ROIs) of the
brain and validate its efficacy by presenting these images to human
participants in an fMRI study. We find that the generated images reliably drive
activity in targeted regions across both low- and high-level visual areas and
across subjects. These results demonstrate that activation maximization can be
successfully applied to DNN-based encoding models. By addressing key
limitations of alternative approaches that require natively generative models,
our approach enables flexible characterization and modulation of responses
across the human visual system.

</details>
