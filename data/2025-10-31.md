...
TL;DR: 本研究提出DBond模型，通过优化镜像肽序列设计来提高测序准确性。该模型基于深度神经网络，整合序列特征、前体离子性质和质谱环境因素来预测肽键断裂，从而筛选易于测序的序列。


<details>
  <summary>Details</summary>
Motivation: 传统非生物存储介质在存储密度和寿命方面存在限制，而镜像肽作为生物存储介质具有高存储密度和长寿命的优势。但镜像肽测序准确性受限于串联质谱数据集稀缺和现有算法的挑战。

Method: 构建了包含513个镜像肽的MiPD513数据集；开发了肽键断裂标记算法(PBCLA)，生成约1250万个标记数据；提出结合多标签和单标签分类的双重预测策略；开发了DBond深度神经网络模型。

Result: 在独立测试集上，单标签分类策略在单肽键和多肽键断裂预测任务中均优于其他方法，为序列优化提供了坚实基础。

Conclusion: 通过优化镜像肽序列设计间接提高测序准确性的方法是可行的，DBond模型为镜像肽生物存储提供了有效的序列优化工具。
...
TL;DR: 该研究提出了算法-结果一致性(AOC)指标，用于量化评估AI预测的肿瘤新抗原与临床疗效之间的转化一致性，为个性化疫苗验证提供标准化框架。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估AI预测模型与临床结果之间转化一致性的统一框架，需要建立量化指标来连接计算预测与临床疗效。

Method: 分析6项黑色素瘤疫苗试验，引入AOC指标(结合模型AUC与临床HR/ORR)，整合机制、经济和监管视角进行系统性评估。

Result: 模拟AOC值在0.42-0.79之间，显示预测与结果一致性存在异质性；高肿瘤突变负荷和克隆新抗原优势与改善的转化一致性相关；AOC>0.7可使ICER降至10万美元/QALY以下。

Conclusion: AOC框架定量连接AI驱动的新抗原预测与临床转化，为未来个性化疫苗验证和监管标准化提供可重复的度量标准。
...
TL;DR: 提出了InputDSA方法，扩展了DSA框架，通过考虑输入驱动来比较动态系统的内在和外部输入动态。


<details>
  <summary>Details</summary>
Motivation: 现有DSA方法只比较系统内在动态，不考虑输入影响，而现实系统很少是自主的，需要考虑输入驱动的影响。

Method: 使用基于子空间识别的动态模式分解控制变体(DMDc)来估计和比较输入和内在动态算子。

Result: InputDSA能成功比较部分观测的输入驱动系统；在真实输入未知时可用替代输入；在RNN中识别出高性能网络动态相似，低性能网络更分散；在神经数据中发现从输入驱动证据积累到内在驱动决策的转变。

Conclusion: InputDSA是用于比较内在动态和外部输入对动态系统影响的稳健高效方法。
...
TL;DR: 本文推导了布尔函数中具有特定数量本质变量和给定渠化性质的递归计数公式，特别确定了非退化渠化布尔函数的数量。


<details>
  <summary>Details</summary>
Motivation: 渠化是复杂系统特别是基因调控网络中的关键组织原则，描述某些输入变量对函数输出的主导控制。然而，渠化和退化性的组合基础仍未完全理解。

Method: 使用递归公式来计数具有规定数量本质变量和给定渠化性质的布尔函数，扩展了早期关于渠化和嵌套渠化函数的枚举结果。

Result: 确定了非退化渠化布尔函数的数量，即所有变量都是本质的且至少有一个变量是渠化的函数。

Conclusion: 该方法为量化渠化在随机布尔函数中的出现频率提供了严格基础，并有助于评估其在生物网络模型中的显著过表达，其中渠化有助于鲁棒性和不同调控角色的出现。
