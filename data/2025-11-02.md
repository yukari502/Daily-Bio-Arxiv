...
TL;DR: 本文提出DBond模型，通过优化镜像肽序列设计间接提高测序准确率，该模型基于深度神经网络预测肽键断裂，选择易于测序的高断裂比序列。


<details>
  <summary>Details</summary>
Motivation: 传统非生物存储介质面临存储密度和寿命限制，镜像肽作为生物存储介质具有优势，但其测序准确率受限于数据集稀缺和算法挑战。

Method: 构建MiPD513数据集，开发肽键断裂标记算法生成标记数据，提出结合多标签和单标签分类的双重预测策略。

Result: 在独立测试集上，单标签分类策略在单肽键和多肽键断裂预测任务中表现优于其他方法。

Conclusion: DBond模型为镜像肽序列优化提供了坚实基础，有助于提高生物存储的测序准确率。
...
TL;DR: 该研究提出了算法-结果一致性(AOC)指标，用于评估AI预测的新抗原疫苗与临床结果之间的转化保真度，分析了6项黑色素瘤疫苗试验数据。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估AI预测模型与临床结果之间转化保真度的一致框架，需要建立定量指标来连接计算预测与临床疗效。

Method: 系统分析了2017-2025年间的6项黑色素瘤疫苗试验，引入AOC指标将模型性能(AUC)与临床疗效(HR/ORR)定量关联，并整合机制、经济和监管视角。

Result: 模拟AOC值在0.42-0.79之间，表明算法预测与观察结果存在异质性一致性。高肿瘤突变负荷和克隆新抗原优势与改善的转化保真度相关，经济模型显示AOC>0.7可使ICER低于10万美元/QALY。

Conclusion: 该框架定量连接了AI驱动的新抗原预测与临床转化，为未来个性化疫苗验证和监管标准化提供了可重复的指标。AOC作为假设生成工具，所有计算基于模拟或汇总试验数据用于演示目的。
...
### [3] [InputDSA: Demixing then Comparing Recurrent and Externally Driven Dynamics](https://arxiv.org/abs/2510.25943)
*Ann Huang,Mitchell Ostrow,Satpreet H. Singh,Leo Kozachkov,Ila Fiete,Kanaka Rajan*

Main category: q-bio.NC

TL;DR: 提出了InputDSA方法，扩展了DSA框架，通过考虑输入驱动来比较动态系统的内在和输入驱动动态，适用于部分观测、噪声数据，并在RNN和神经数据中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有DSA方法只比较系统内在动态，未考虑输入影响，而现实系统多为非自治的，输入对动态有重要影响，需要开发能同时比较内在和输入驱动动态的方法。

Method: 基于子空间识别的动态模式分解控制变体(DMDc)估计和比较输入和内在动态算子，扩展DSA框架，支持使用替代输入处理未知真实输入情况。

Result: InputDSA能成功比较部分观测、输入驱动系统；替代输入不会显著影响相似性估计；高表现RNN动态相似，低表现RNN更分散；在神经数据中识别出从输入驱动证据积累到内在驱动决策的转变。

Conclusion: InputDSA是稳健高效的方法，能比较内在动态和外部输入对动态系统的影响，适用于各种应用场景。
...
### [4] [On the number of non-degenerate canalizing Boolean functions](https://arxiv.org/abs/2510.26556)
*Claus Kadelka*
...
Method: 使用递归公式来枚举具有规定数量本质变量和渠化性质的布尔函数，扩展了先前关于渠化和嵌套渠化函数的枚举结果。

Result: 确定了非退化渠化布尔函数的数量，为量化渠化在随机布尔函数中的出现频率提供了严格基础。

Conclusion: 该方法能够评估渠化在生物网络模型中的显著过度表达，这有助于系统的鲁棒性和不同调控角色的出现。
