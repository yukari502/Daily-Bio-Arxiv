...
Motivation: 数学模型在生态学和流行病学中支持推断和预测，但结果取决于估计框架。本研究旨在比较贝叶斯和频率主义方法在不同生物模型和数据条件下的性能差异。

Method: 使用三种生物模型和四个数据集：Lotka-Volterra捕食者-猎物动态、广义逻辑模型和SEIUR流行病模型。首先评估结构可识别性，然后使用MAE、MSE、95%预测区间覆盖率和WIS四个指标评估实际可识别性和预测性能。频率主义方法使用QDF进行非线性最小二乘拟合和参数自举法，贝叶斯方法使用BFF进行哈密顿蒙特卡洛采样。

Result: 频率主义推断在数据丰富且完全可观测时表现最佳，而贝叶斯推断在潜在状态不确定性高且数据稀疏时表现更优，特别是在SEIUR COVID-19模型中。结构可识别性分析显示完全可观测性对两种框架都有利，而有限可观测性会限制参数恢复。

Conclusion: 研究为根据数据丰富度、可观测性和不确定性需求选择推断框架提供了指导：数据丰富时选择频率主义方法，数据稀疏且不确定性高时选择贝叶斯方法。
...
TL;DR: 该研究探讨了数学模型选择对化疗和放疗方案优化的影响，发现不同模型假设会导致矛盾的治疗建议，强调需要基于偏差分析和不确定性量化来选择模型。


<details>
  <summary>Details</summary>
Motivation: 随着个性化治疗和数字孪生在癌症治疗中的应用，需要深入理解支撑这些优化策略的数学模型，以避免模型假设带来的偏见和不可推广的建议。

Method: 研究了三种化疗模型（对数杀伤、Norton-Simon、Emax）和三种放疗模型（线性二次、增殖饱和指数、连续死亡率），分析它们对优化治疗方案的影响。

Result: 发现模型中的假设严重影响了最优治疗的剂量和顺序，可能导致相互矛盾的结果，不同模型形式在适应性治疗环境中的预测也存在显著差异。

Conclusion: 模型选择应基于完整的偏差分析、敏感性分析、参数可识别性和不确定性量化，而不仅仅是信息准则，以确保预测的稳健性和可推广性。
...
TL;DR: ProtT Affinity是一个仅使用序列信息的蛋白质结合亲和力预测模型，结合ProtT5嵌入和轻量级Transformer架构，在PDBBind数据库上训练，性能与多种现有方法相当。


<details>
  <summary>Details</summary>
Motivation: 在缺乏可靠结构信息的情况下，直接从序列预测蛋白质-蛋白质复合物的结合亲和力仍然是一个挑战。

Method: 结合ProtT5嵌入和轻量级Transformer架构，在PDBBind数据库的同源过滤子集上进行训练和评估。

Result: 在两个独立测试集上分别达到0.628和0.459的Pearson相关系数，性能与多种广泛使用的方法相当。

Conclusion: 大型蛋白质语言模型捕获了与结合能量相关的特征，这些特征可用于大规模近似亲和力趋势，为结构数据缺失或不确定时提供了实用替代方案。
...
TL;DR: 提出了多种自适应内存优化机制，用于并行化染色体级生物信息学工作流，包括符号回归模型预测内存使用、动态调度器优化任务打包、静态调度器优化处理顺序，从而减少内存溢出并提升执行效率。


<details>
  <summary>Details</summary>
Motivation: 大规模基因组工作流在精准医疗中处理数十到数百GB的数据，导致内存峰值高、磁盘I/O密集、任务因内存不足而失败。静态资源分配方法难以处理不同染色体的内存需求变化，导致资源利用率低和运行时间长。

Method: 1. 开发符号回归模型估计每个染色体的内存消耗，引入插值偏差保守最小化过度分配；2. 提出动态调度器使用多项式回归模型自适应预测RAM使用，将任务打包视为背包问题优化批处理；3. 提出静态调度器优化染色体处理顺序以最小化峰值内存同时保持吞吐量。

Result: 在模拟和真实基因组流程中的评估表明，所提方法能够减少内存溢出并在线程间平衡负载，从而实现更快的端到端执行。

Conclusion: 这些方法为优化大规模基因组工作流提供了新机制，展示了在减少内存过度分配和提高执行效率方面的潜力。
...
TL;DR: 开发了一个基于弱监督分割和ResNet50分类器的自动化图像处理流程，用于在帕金森病和中度路易体疾病的全玻片图像中分割和分类α-突触核蛋白聚集物，能够区分路易体和神经突等主要形态，平衡准确率达80%。


<details>
  <summary>Details</summary>
Motivation: 帕金森病与α-突触核蛋白错误折叠聚集物（形成路易体和神经突）的积累相关，自动分析免疫组织化学组织病理学图像有助于更好地理解这些聚集物的空间组织。

Method: 基于弱监督分割和ResNet50分类器的自动化图像处理流程，对免疫组织化学标记变异性具有鲁棒性，用于分割和分类全玻片图像中的α-突触核蛋白聚集物。

Result: 能够区分主要聚集物形态（包括路易体和神经突），平衡准确率达到80%。

Conclusion: 该框架为大规模表征α-突触核蛋白聚集物在明场免疫组织化学组织中的空间分布和异质性，以及研究其与周围细胞（如小胶质细胞和星形胶质细胞）之间知之甚少的关系奠定了基础。
