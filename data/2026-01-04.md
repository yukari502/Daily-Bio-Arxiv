...
TL;DR: 提出了Process Bigraphs框架，用于构建多尺度生物模型，通过定义过程接口、层次数据结构和组合模式来改善模型清晰度、重用性和可扩展性，并发布了开源实现Vivarium 2.0。


<details>
  <summary>Details</summary>
Motivation: 构建多尺度生物模型需要集成独立开发的子模型，但现有工具很少明确指定模型接口（哪些变量被读写、如何转换、如何同步更新），导致模型集成困难。

Method: 提出Process Bigraphs框架，将Vivarium软件的架构原则推广为共享规范，定义过程接口、层次数据结构、组合模式和编排模式。框架包括bigraph-schema（层次JSON数据类型）、process-bigraph（定义过程接口和执行复合模拟）、bigraph-viz（可视化系统结构和数据流）。

Result: 开发了Vivarium 2.0作为Process Bigraph框架的开源实现，并展示了Spatio-Flux应用，该库整合了动力学ODE、动态通量平衡分析和空间过程，用于微生物生态系统模拟。

Conclusion: Process Bigraphs框架通过明确的过程接口和组合规范，显著提高了多尺度生物模型的清晰度、重用性和可扩展性，为多尺度建模的新兴标准提供了重要参考。
...
TL;DR: 该论文扩展了细胞大小控制的"加法器"模型，引入生长饱和效应和多步调控机制，推导出细胞大小分布的精确解析表达式，证明生长饱和会增加平均细胞大小但减少波动，同时保持加法器特性。


<details>
  <summary>Details</summary>
Motivation: 现有细胞大小控制的加法器模型假设指数生长，但实际细胞生长可能存在饱和效应。需要扩展理论框架，考虑更一般的生长规律和细胞周期调控的多步特性。

Method: 提出基于Hill函数的广义生长规律模型，引入生长饱和效应；建立随机多步加法器模型，模拟细胞通过内部调控阶段进行分裂的过程；推导细胞大小分布的精确解析表达式。

Result: 更强的生长饱和会增加稳态下的平均细胞大小，同时略微减少波动；加法器特性在生长饱和下仍然保持；生长饱和同时影响单细胞大小统计和群体变异性。

Conclusion: 该研究提供了一个连接多步加法器机制与增殖动力学的广义框架，将大小控制理论扩展到指数生长之外，强调生长规律而非简单缩放是减少大小变异性的原因。
...
TL;DR: 重新设计水浴式PCR，结合自动化液体处理技术，在密封移液器吸头内完成扩增，适用于DNA数据存储等新兴应用。


<details>
  <summary>Details</summary>
Motivation: 传统PCR仪在DNA数据存储等新兴应用中面临自动化程度不足、污染控制困难和成本效益问题，需要更适应现代需求的解决方案。

Method: 采用水浴式PCR原理，结合机器人液体处理技术，在密封移液器吸头内进行扩增，通过自动浸入和移出单一温控油浴完成变性、退火和延伸步骤。

Result: 该系统扩增效率和测序保真度与高性能PCR仪相当，同时减少试剂浪费、通过完全吸头隔离降低污染风险，并能实现样品完全回收。
...
TL;DR: 这篇综述论文系统回顾了跑步过程中的肌肉协同作用研究，总结了神经控制理论、分解方法、影响因素以及临床应用，指出了当前方法的局限性并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 跑步作为人类基本运动形式，是评估神经肌肉控制和下肢协调的关键任务。近年来基于表面肌电图的肌肉协同分析已成为该领域重要方法，但存在方法学差异和验证不足的问题，需要系统梳理现有研究并指导未来方向。

Method: 采用文献综述方法，系统分析跑步肌肉协同作用相关研究。涵盖核心神经控制理论（如模块化控制、最优控制）、常用分解方法（PCA、ICA、FA、NMF）和新兴的自编码器方法，并综合不同因素对协同模式的影响。

Result: 研究发现跑步过程中下肢肌肉协同的数量和基本结构相对稳定，但空间肌肉权重和运动原语具有高度可塑性，对任务需求、疲劳和病理状态敏感。不同人群（膝骨关节炎、髌股疼痛、中风）表现出特征性协同模式改变。

Conclusion: 当前肌肉协同分析存在方法学变异性和验证不足的局限。未来应优先发展标准化处理协议、多源神经肌肉骨骼数据整合、非线性建模和纵向干预研究，以更好地应用于运动生物力学、运动训练和康复医学。
...
TL;DR: friends.test方法通过检测实体交互中的结构断点来识别特异性交互，使用基于排名的表示确保对异构数据的鲁棒性，并自动定位特异性交互与背景活动的边界。


<details>
  <summary>Details</summary>
Motivation: 在药物警戒、转录组学等多个领域中，分析两个不同集合之间的交互矩阵至关重要。但并非所有交互都具有同等信息价值：与少数特定生物过程相关的标记基因比与大多数观察过程相关的高表达非特异性基因更具信息性。由于背景连接的存在，识别这些特异性交互具有挑战性，且数据来源的异质性使得无法建立通用的识别标准。

Method: 提出friends.test方法，通过检测实体交互中的结构断点来识别特异性。使用基于排名的交互矩阵表示确保对异构数据的鲁棒性，允许整合不同来源的数据。采用模型拟合自动定位特异性交互与背景活动之间的边界。

Result: 在GSE112026头颈癌跨国数据上验证了friends.test的适用性。该方法提供了计算高效的R语言实现，可在GitHub上获取。

Conclusion: friends.test为识别两个集合之间的特异性交互提供了一种有效方法，能够处理数据异质性并自动区分特异性交互与背景活动，在生物信息学等领域具有广泛应用前景。
...
TL;DR: 论文提出从输入输出角度分析生物系统的"动态表型"，强调瞬态响应特征对网络结构的约束作用，特别关注非单调系统和拮抗通路在产生适应性响应中的作用。


<details>
  <summary>Details</summary>
Motivation: 传统稳态或剂量响应分析无法充分捕捉生物系统中关键的瞬态响应特征（如过冲、双相动力学、适应动力学等），而这些动态表型往往决定表型结果。需要新的分析框架来理解、测试和逆向工程生物网络。

Method: 采用控制理论概念（单调性、等变性、输入输出分析）作为精确数学工具，分析非一致前馈（IFF）基序在产生非单调和适应性响应中的作用，通过瞬态数据验证模型类别。

Result: 展示了IFF基序在从分子信号到免疫调节和种群动力学的多个生物组织层次上产生非单调和适应性响应的机制，单调性提供了从瞬态数据单独证伪整个模型类的可能性。

Conclusion: 应该将重点从渐近行为转向瞬态和输入驱动的动力学，作为理解、测试和逆向工程生物网络的主要视角，控制理论概念可作为精确的数学工具用于生物模型区分。
...
TL;DR: 开发了一个扩展的动力学平均场理论框架，用于分析包含表观遗传修饰的基因调控网络，将DNA甲基化和组蛋白修饰的动态项引入Hopfield网络模型，将高维GRN动力学简化为有效随机方程。


<details>
  <summary>Details</summary>
Motivation: 需要建立一个理论框架来定量分析表观遗传修饰（如DNA甲基化和组蛋白修饰）如何调控基因表达，并理解这些调控在发育过程和细胞命运决定中的作用。

Method: 基于Hopfield网络模型与自旋玻璃系统的类比，引入代表DNA甲基化和组蛋白修饰的动态项，构建扩展的动力学平均场理论框架，将高维基因调控网络动力学简化为有效随机方程。

Result: 该框架能够描述表观遗传调控系统中稳定和振荡状态的特征，为基因调控动力学与表观遗传控制之间的联系提供了可处理且定量的分析方法。

Conclusion: 该扩展的动力学平均场理论框架为理解发育过程和细胞命运决定提供了新的理论见解，实现了基因调控动力学与表观遗传控制的定量连接。
...
TL;DR: 开发了一个基于开源工具的大规模微有限元框架，用于完整兔股骨的高分辨率生物力学分析，通过实验验证实现了微米尺度的材料属性校准


<details>
  <summary>Details</summary>
Motivation: 高分辨率基于体素的微有限元模型能够详细研究骨力学，但在解剖学相关尺度上仍然计算挑战大，需要开发可扩展的框架

Method: 使用MIA聚类算法从μCT数据分割骨几何，转换为基于体素的μFE网格，利用开源MFEM库求解大规模线性弹性系统，并与商业求解器验证，结合数字图像相关测量进行实验校准

Result: 成功求解了超过8亿自由度的模型，发现40μm体素尺寸在保持边界位移和主应变分布的同时显著降低计算成本，通过实验校准获得了微米尺度的有效骨材料属性

Conclusion: 基于开源工具的大规模实验验证微有限元建模是可行的，为骨力学和与治疗相关风险的临床前评估提供了坚实基础
