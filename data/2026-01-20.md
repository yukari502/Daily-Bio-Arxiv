...
Motivation: 抑郁症是严重影响生活质量的心理健康障碍，早期诊断能显著提高治疗效果。传统诊断方法依赖主观的自我报告评估，缺乏可靠性和客观性，因此需要开发客观准确的抑郁症检测技术。

Method: 提出深度学习框架，使用EEG信号（不受外部行为因素影响）检测抑郁状态。结合卷积神经网络（CNN）和门控循环单元（GRU）共同提取EEG记录的时空特征，然后应用最小冗余最大相关性（MRMR）算法选择最具信息量的特征，最后使用全连接神经网络进行分类。

Result: 所提出的模型在准确识别抑郁状态方面表现出高性能，总体准确率达到98.74%。通过有效整合时空信息并采用优化的特征选择，该方法显示出作为临床应用的可靠工具的潜力。

Conclusion: 该框架不仅能够准确早期检测抑郁症，还有潜力支持改进的治疗策略和患者预后，为抑郁症诊断提供了客观、准确的替代方案。
...
TL;DR: 开发了结合肿瘤负荷与血液标志物动力学的机制模型TALN-k，并集成机器学习框架TALN-kML，用于预测非小细胞肺癌患者的总生存期，在临床试验数据上表现优于现有模型。


<details>
  <summary>Details</summary>
Motivation: 预测非小细胞肺癌患者的总生存期对临床决策和药物开发至关重要。虽然肿瘤和血液标志物动力学存在内在联系，但它们的联合动态及其与总生存期的关系尚不清楚。

Method: 开发了TALN-k机制模型，通过耦合微分方程捕捉肿瘤负荷与三种关键血液标志物（白蛋白、乳酸脱氢酶、中性粒细胞）动力学的相互作用。将该模型与机器学习框架TALN-kML结合用于总生存期预测。使用非线性混合效应模型估计参数，并在接受atezolizumab单药或联合治疗的NSCLC患者临床试验数据上进行训练和验证。

Result: TALN-k成功描述了个体和群体水平的标志物动力学，揭示了肿瘤与血液标志物之间的复杂相互作用，在修正BIC和对数似然指标上显著优于现有经验模型。TALN-kML模型在总生存期预测方面优于非耦合经验模型，C指数（0.74 vs 0.72）、12个月AUC（0.83 vs 0.79）和准确率（0.77 vs 0.76）均有改善。

Conclusion: 这种机制学习方法提供了一个可解释的模型，通过联合整合肿瘤和血液标志物动力学，改善了NSCLC纵向数据描述和生存预测。该方法为个性化治疗策略和药物开发优化提供了有前景的途径。
