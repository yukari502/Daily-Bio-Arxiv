...
TL;DR: FUNGAR是一个用于直接从短读长测序数据中快速识别抗真菌耐药基因和突变的开源流程，特别针对宏基因组样本设计。


<details>
  <summary>Details</summary>
Motivation: 抗真菌耐药性已成为临床和环境健康领域日益严重的全球性问题。直接从测序读长（特别是宏基因组样本）中检测已知耐药突变仍然是一个主要挑战。与细菌病原体相比，真菌病原体常被忽视，现有工具大多针对细菌设计，而针对真菌的工具通常需要组装基因组。在宏基因组数据集中，基于组装的策略可能因基因组碎片化、低丰度物种或耐药位点恢复不完整而导致大量信息丢失。

Method: FUNGAR使用DIAMOND进行翻译比对，并结合FungAMR数据库的精选数据，在所有六个开放阅读框中检测氨基酸替换。该流程生成结构化、可重复的报告，将检测到的变异与相关抗真菌药物关联起来，并可轻松定制以适应新物种或数据库。

Result: 开发了FUNGAR开源流程，能够直接从短读长数据中识别抗真菌耐药基因和突变，特别适用于宏基因组样本分析。

Conclusion: FUNGAR填补了真菌耐药性检测工具的空缺，为直接从宏基因组测序读长中检测抗真菌耐药突变提供了高效、可定制的解决方案，有助于改善真菌耐药性监测和研究。
...
TL;DR: 作者提出了一个系统评估框架来评估单细胞基础模型的机制可解释性，发现注意力模式编码了结构化的生物学信息，但并未为扰动预测提供增量价值，基因级基线模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 当前单细胞基础模型（如scGPT和Geneformer）的机制可解释性缺乏系统评估标准，需要建立可重复的质量控制标准来评估这些模型是否真正捕捉到了生物学机制。

Method: 开发了一个包含37个分析、153个统计测试、4种细胞类型、2种扰动模式的系统评估框架，应用于scGPT和Geneformer模型，分析注意力模式、相关性边、因果消融等。

Result: 注意力模式编码了结构化生物学信息（早期层编码蛋白质-蛋白质相互作用，晚期层编码转录调控），但这对扰动预测没有增量价值；基因级基线模型（AUROC 0.81-0.88）优于注意力边和相关性边（AUROC 0.70）；细胞状态分层可解释性（CSSI）将GRN恢复提高了1.85倍。

Conclusion: 该框架为领域建立了可重复的质量控制标准；注意力模式虽然编码生物学结构，但并未提供预测优势；基因级特征在扰动预测中占主导地位；CSSI方法解决了注意力特定的缩放问题。
...
TL;DR: 该论文综述了利用系统发育重建方法构建分子时间线和网络，揭示蛋白质起源与演化历史，重点关注结构域家族的时间线、折叠与招募过程在进化中的核心作用。


<details>
  <summary>Details</summary>
Motivation: 尽管蛋白质的起源和演化仍然神秘，但进化基因组学和系统生物学的发展使得我们能够从历史角度探索蛋白质和蛋白质组的结构、功能和组织的演化历程。

Method: 使用现代系统发育重建方法构建分子时间线和时间变化网络，分析结构域家族的时间线，研究蛋白质折叠和招募过程。

Result: 揭示了代谢的爆发性出现、翻译的晚期兴起、核糖体蛋白与rRNA的共进化、核糖体出口隧道的晚期发展，以及结构域早期出现和结构域组合的晚期"大爆炸"模式。

Conclusion: 折叠和招募是进化进程中的两个核心过程：折叠增加蛋白质持久性，招募促进多样性。蛋白质进化在时间上反映了折叠过程，通过将超二级结构组合成结构域、发展翻译机制以促进折叠速度和稳定性，以及通过建立长距离相互作用来增强结构复杂性。
...
TL;DR: 开发基于MALDI-TOF质谱和抗生素耐药性模式的机器学习框架，作为全基因组测序的快速低成本替代方案，用于医院感染暴发检测


<details>
  <summary>Details</summary>
Motivation: 全基因组测序作为医院感染暴发检测的金标准成本高、耗时长，难以在临床实验室常规实施，需要寻找快速、经济有效的替代方案
...
Conclusion: MALDI-TOF质谱和抗生素耐药性模式可作为全基因组测序的可行替代方案，为医院感染暴发检测提供更快速、经济有效的监测工具
...
TL;DR: 提出U-FedTomAtt超轻量联邦学习框架，用于资源受限分布式环境下的番茄病害识别，模型仅245.34K参数，在多个数据集上达到99%以上准确率。


<details>
  <summary>Details</summary>
Motivation: 传统集中式训练存在通信开销、隐私风险和延迟问题，而边缘设备需要轻量级网络在有限资源下运行。需要开发适用于分布式农业环境的隐私保护高效番茄病害识别方案。

Method: 1) 提出超轻量神经网络，包含扩张瓶颈模块和线性变换器；2) 引入局部-全局残差注意力模块减少精度损失；3) 提出联邦双自适应权重聚合算法提升全局模型精度。

Result: 在模拟联邦设置下，在SLIF-Tomato数据集上达到99.10% Top-1准确率和99.23% F1分数，在PlantVillage数据集上达到99.15% Top-1准确率和98.97% F1分数。

Conclusion: U-FedTomAtt框架在资源受限的分布式环境中有效实现了番茄病害识别，平衡了模型轻量化和识别精度，为智能农业提供了实用的隐私保护解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基因组基础模型主要依赖掩码语言建模或下一词预测，虽然能捕捉局部基因组语法和精细基序模式，但缺乏对全局功能上下文的理解，导致表征缺少生物学全局视角

Method: 引入JEPA-DNA框架，将联合嵌入预测架构与生成目标结合，通过潜在接地将标记级恢复与潜在空间预测目标耦合，监督CLS标记预测掩码基因组片段的高层功能嵌入而非单个核苷酸

Result: 在多样化基因组基准测试中，JEPA-DNA在监督和零样本任务中始终优于纯生成基线，提供更稳健和生物学接地的表征

Conclusion: JEPA-DNA为理解基因组字母和序列底层功能逻辑的基础模型提供了可扩展路径，通过整合预测性和生成性目标实现了更好的全局功能理解
