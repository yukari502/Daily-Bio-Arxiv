...
TL;DR: 该研究提出了一种基于基因组基础模型嵌入和局部激活模式保留的跨物种抗生素耐药性预测方法，解决了传统方法在跨物种评估中性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 跨物种抗生素耐药性预测本质上是一个分布外泛化问题，因为不同细菌物种依赖不同的耐药机制。传统方法在物种内表现良好但在跨物种评估中性能崩溃，需要能够保留可转移生物学信号而非放大系统发育捷径的方法。

Method: 使用严格的物种留出协议评估可解释k-mer基线方法；研究基因组基础模型嵌入（Evo-1-8k-base），基于激活尺度、各向同性、有效秩和跨种子稳定性等诊断指标选择层；将每个窗口嵌入视为有序多元信号，应用MiniRocket总结多尺度局部激活模式而非依赖全局池化。

Result: 分析发现深层存在稳定性边界，靠近该边界提取的嵌入为下游预测提供更稳健的表示；保留局部激活模式的聚合策略显著改善了当耐药机制局部化时的泛化能力。

Conclusion: 聚合策略在跨物种AMR预测中起核心作用，保留局部激活模式能够显著改善泛化性能，特别是当耐药机制局部化时。这为跨物种抗生素耐药性预测提供了更稳健的方法。
...
TL;DR: GGE是一个用于单细胞基因表达生成模型评估的开源Python框架，提供标准化评估指标和生物学验证方法，解决当前评估方法不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 单细胞基因表达生成模型快速发展，但缺乏标准化评估框架。当前评估实践存在指标实现不一致、超参数选择不可比、缺乏生物学基础指标等问题，阻碍了公平比较和可重复性研究。

Method: 开发了Generated Genetic Expression Evaluator (GGE)开源框架，提供全面的分布度量套件，具有明确的计算空间选项，并通过差异表达基因(DEG)分析和扰动效应相关性进行生物学验证，支持标准化报告和可重复基准测试。

Result: 通过对单细胞生成建模文献的广泛分析，发现不存在标准化评估协议，不同方法报告的指标在不同空间使用不同超参数计算，不可比较。研究表明指标值因实现选择而有显著差异，凸显了标准化的迫切需求。

Conclusion: GGE框架能够实现生成方法的公平比较，加速扰动响应预测、细胞身份建模和反事实推理等领域的进展，为单细胞生成模型评估提供了标准化解决方案。
...
TL;DR: ELISA是一个将单细胞RNA测序数据转化为可解释生物学假设的AI框架，通过结合scGPT表达嵌入、BioBERT语义检索和LLM解释，实现交互式单细胞发现。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统缺乏对转录组学表征的直接访问，而表达基础模型对自然语言不透明，这阻碍了将单细胞RNA测序数据转化为机制性生物学假设的过程。

Method: 开发了ELISA框架，统一scGPT表达嵌入与BioBERT语义检索和LLM介导的解释。包含自动查询分类器、通路活性评分、配体-受体相互作用预测、条件感知比较分析和细胞类型比例估计等模块。

Result: 在六个不同的scRNA-seq数据集上，ELISA在细胞类型检索方面显著优于CellWhisperer，特别是在基因特征查询上表现优异。能够复现已发表的生物学发现，生成候选假设。

Conclusion: ELISA成功弥合了转录组数据探索与生物学发现之间的差距，提供了一个可解释的框架，将单细胞RNA测序数据转化为机制性生物学假设。
...
TL;DR: 该论文分析了数据驱动方程发现中的数值病态问题，特别是在生物系统识别中，当候选函数高度相关时会导致模型恢复不稳定，并研究了正交多项式基在不同数据分布下的表现。


<details>
  <summary>Details</summary>
Motivation: 数据驱动方法从时间序列数据中发现控制方程是理解复杂生物系统的强大框架，但库基方法面临一个关键挑战：当候选函数高度相关时会出现数值病态问题。采样受限和特定候选库选择会导致强多重共线性和数值不稳定，使得测量噪声可能导致恢复的模型差异很大，难以准确识别真实动态系统。

Method: 对稀疏识别生物动态中的病态问题进行系统分析，使用系统生物学的基准模型。研究涉及少至两三个项的线性组合如何表现出强多重共线性和极大条件数。进一步研究正交多项式基是否始终解决病态问题，以及当数据分布偏离正交基相关权重函数时，正交基可能比单项式库表现更差。

Result: 研究表明，即使只有两三个项的线性组合也可能表现出强多重共线性和极大条件数。正交多项式基不能始终解决病态问题，当数据分布偏离正交基相关权重函数时，正交基可能比单项式库表现更差。但当数据采样分布与正交基对应的适当权重函数对齐时，数值条件改善，正交多项式基可以在两个基准模型中提高模型恢复准确性。

Conclusion: 数值病态是数据驱动方程发现中的重要问题，特别是在生物系统识别中。正交多项式基的性能取决于数据分布与基函数权重函数的对齐程度。当数据采样与适当权重函数匹配时，正交基能改善数值条件并提高模型恢复准确性，这为改进生物动态系统识别方法提供了重要见解。
...
TL;DR: 提出了一种无需炼金术中间态的直接结合自由能计算方法DBFE，在隐式溶剂中仅需复合物模拟，性能优于传统方法且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 传统绝对结合自由能方法需要大量炼金术中间态模拟，计算成本高，限制了其在虚拟筛选中的应用。

Method: 开发了DBFE方法，这是一种隐式溶剂中的端态ABFE方法，无需炼金术中间态，仅需受体、配体和复合物的模拟，其中受体和配体模拟可预先计算并跨化合物复用。

Result: 在宿主-客体基准测试中优于OBC2双去耦方法，在蛋白质-配体基准测试中与OBC2 MM/GBSA性能相当，且每个配体仅需一次复合物模拟。

Conclusion: DBFE是一种高效准确的结合自由能预测方法，特别适合虚拟筛选工作流，代码已公开。
...
TL;DR: 提出基于竞争阻断电路的可扩展三进制DNA加法器架构，通过双协同优化策略提升单比特计算能力，结合动态浓度调整扩展计算位宽，实现17位加法运算


<details>
  <summary>Details</summary>
Motivation: 现有DNA加法器主要关注二进制加法，存在进位信息易衰减和二进制系统表达能力有限的问题，限制了计算规模扩展

Method: 引入创新性竞争阻断电路，采用双协同优化策略增强单比特计算能力，结合动态浓度调整机制有效扩展计算位宽

Result: CB电路成功输出三进制全加器比特并实现10位加法，通过CA策略进一步扩展至17位加法，相比先前二进制加法器显著提升分子计算规模

Conclusion: 该研究为推进DNA计算技术提供了新的方法论基础，为可扩展数字计算应用提供了有前景的潜力
...
### [7] [A Multi-Label Temporal Convolutional Framework for Transcription Factor Binding Characterization](https://arxiv.org/abs/2603.12073)
...
<details>
  <summary>Details</summary>
Motivation: 转录因子通过复杂协同机制调控基因表达，但现有方法主要关注单个TF的二元分类，缺乏对多个TF相互作用的全面分析，需要开发能捕捉TF间协同作用的预测模型。

Method: 将DNA转录因子结合位点识别构建为多标签分类问题，采用时间卷积网络（TCN）深度学习模型，从公共数据库获取DNA序列数据，预测多个TF的结合谱。

Result: TCN模型能够可靠预测多个TF的结合模式，捕捉TF间的相关性，识别出与已知TF相互作用一致的生物学有意义基序和共结合模式，同时提示新的TF关系和协同作用。

Conclusion: 多标签学习方法不仅能实现可靠的预测性能，还能揭示TF间的生物学相互作用和协同调控机制，为理解转录因子结合逻辑提供了新视角。
...
### [8] [Single molecule localization microscopy challenge: a biologically inspired benchmark for long-sequence modeling](https://arxiv.org/abs/2603.11296)
...
<details>
  <summary>Details</summary>
Motivation: 状态空间模型在长序列建模任务中表现出色，但现有评估主要局限于合成基准和语言/音频领域，缺乏对生物成像中稀疏随机时间过程的研究。

Method: 引入单分子定位显微镜挑战数据集，包含10个SMLM模拟，涵盖dSTORM和DNA-PAINT模态，具有不同超参数，用于评估状态空间模型在生物真实时空点过程数据上的性能。

Result: 评估发现状态空间模型性能随着时间不连续性增加而显著下降，揭示了在建模重尾闪烁动态方面的基本挑战。
...
### [9] [Leveraging Phytolith Research using Artificial Intelligence](https://arxiv.org/abs/2603.11476)
...
TL;DR: Sorometry：一个基于AI的端到端高通量植硅体分析平台，结合2D图像和3D点云数据，实现自动分类和植物来源预测


<details>
  <summary>Details</summary>
Motivation: 传统植硅体分析方法依赖耗时的人工显微镜观察，严重限制了研究效率。需要开发自动化、高通量的解决方案来克服这一瓶颈。

Method: 开发了Sorometry平台，包括：1）处理z-stack光学显微镜扫描生成同步的2D正射影像和3D点云；2）构建多模态融合模型（ConvNeXt处理2D图像，PointNet++处理3D点云）；3）集成图形用户界面供专家标注和审核；4）采用贝叶斯有限混合模型预测植物来源贡献。

Result: 在玻利维亚亚马逊的参考集和考古样本测试中：融合模型在24个诊断形态类型上达到77.9%的全局分类准确率，分割质量达84.5%。3D数据对区分复杂形态类型（如禾本科短细胞植硅体）至关重要。平台成功识别了玉米和棕榈等特定植物。
...
### [10] [Realizing Common Random Numbers: Event-Keyed Hashing for Causally Valid Stochastic Models](https://arxiv.org/abs/2603.11084)
*Vince Buffalo,Carl A. B. Pearson,Daniel Klein*

Main category: stat.ME

TL;DR: 标准PRNG实现会导致因果不一致的配对反事实比较，提出使用基于计数器的RNG和事件标识符来修复


<details>
  <summary>Details</summary>
Motivation: 基于代理的模型（ABMs）通过配对反事实模拟来估计因果处理效应。常用的方差减少技术是公共随机数（CRNs），通过共享相同的随机输入来耦合不同干预场景的重复实验。但实践中CRNs的实现依赖于一个关键假设：相同的抽取索引对应不同场景中的相同建模事件。状态伪随机数生成器（PRNGs）在干预改变模拟执行路径时会违反这一假设。

Method: 通过结构因果模型（SCMs）的视角形式化分析问题，提出将基于计数器的随机数生成器（如Philox/Threefry）与事件标识符相结合的方法。这种方法使随机数生成与模拟执行顺序解耦，通过使随机抽取成为调用它们的特定建模事件的显式函数。

Result: 标准PRNG实践即使在机制规范正确的情况下也会产生因果不一致的配对反事实比较。提出的方法恢复了SCMs假设的稳定事件索引外生结构，解决了执行路径依赖的抽取索引问题。

Conclusion: 状态PRNGs的执行路径依赖抽取索引不仅影响方差减少，更代表了ABMs意图编码的科学因果结构与程序级因果结构之间的根本性不匹配。提出的基于计数器的RNG与事件标识符的组合方法能够恢复因果一致性，确保配对反事实比较的有效性。
...
### [11] [Hybrid eTFCE-GRF: Exact Cluster-Size Retrieval with Analytical p-Values for Voxel-Based Morphometry](https://arxiv.org/abs/2603.11344)
*Don Yin,Hao Chen,Takeshi Miki,Boxing Liu,Enyu Yang*

Main category: eess.IV

TL;DR: 提出了一种结合eTFCE的并查集精确聚类大小检索与pTFCE的解析高斯随机场推断的新方法，实现了无需置换测试的快速精确TFCE分析。


<details>
  <summary>Details</summary>
Motivation: 传统TFCE方法存在速度瓶颈：置换测试的TFCE在大数据集上计算成本过高，而pTFCE虽然使用解析方法但存在阈值离散化问题，eTFCE解决了离散化但仍需置换测试。

Method: 结合eTFCE的并查集数据结构（用于精确聚类大小检索）和pTFCE的解析高斯随机场理论。并查集在排序体素上单次遍历构建聚类层次结构，支持任意阈值的精确大小查询；GRF理论将这些大小转换为解析p值，无需置换测试。

Result: 在合成数据上：FWER控制在标称水平（0/200零假设拒绝，95% CI [0.0%, 1.9%]）；功效与基线pTFCE匹配（Dice ≥ 0.999）；平滑度误差低于1%；一致性r > 0.99。在UK Biobank和IXI数据集上，显著性图形成参考R pTFCE的严格子集，支持保守误差控制。计算速度显著提升：基线版本比R pTFCE快75倍，混合版本快4.6倍，两者都比置换TFCE快1000倍以上。

Conclusion: 提出的方法实现了快速、精确的TFCE分析，无需置换测试，在保持统计功效的同时大幅提升计算效率，为大规模神经影像分析提供了实用工具。
...
TL;DR: Nyxus是一个为大规模2D/3D图像数据设计的可扩展特征提取库，支持CPU/GPU并行计算，提供Python包、命令行工具、Napari插件和容器化部署，适用于生物医学领域如放射组学和细胞分析。


<details>
  <summary>Details</summary>
Motivation: 现代成像仪器产生TB到PB级数据，现有图像分析算法在处理大规模数据集时效率不足，深度学习改进了区域分割但领域特定特征提取库分散，难以比较性能和准确性。

Method: 开发Nyxus特征提取库，采用可扩展的out-of-core架构，支持2D/3D图像数据，覆盖生物医学多个领域特征，提供多种使用方式（Python包、CLI、Napari插件、OCI容器），支持CPU/GPU计算。

Result: Nyxus提供全面的特征集，经过严格测试符合标准，支持程序化调优特征集以获得最佳计算效率或覆盖范围，适用于新型机器学习和深度学习应用。

Conclusion: Nyxus解决了大规模图像数据处理的计算瓶颈，统一了特征提取标准，为不同技能水平的用户提供灵活访问方式，支持云端和超算工作流，推动了特征提取方法学的发展。
...
### [13] [Framing local structural identifiability and observability in terms of parameter-state symmetries](https://arxiv.org/abs/2603.11387)
*Johannes G. Borgqvist,Alexander P. Browning,Fredrik Ohlsson,Ruth E. Baker*

Main category: math.DS

TL;DR: 提出参数-状态对称性子类分析机械模型的结构可识别性和可观测性，证明可识别参数组合和可观测状态对应所有参数-状态对称性的通用不变量


<details>
  <summary>Details</summary>
Motivation: 为分析具有观测输出的状态依赖ODE机械模型，需要一种统一的对称性方法来研究局部结构可识别性和可观测性

Method: 引入参数-状态对称性子类，这些对称性在参数和状态上作用，同时保持每个时间点的观测输出不变

Result: 证明局部结构可识别参数组合和局部结构可观测状态对应给定模型所有参数-状态对称性的通用不变量

Conclusion: 在四个先前研究的机械模型上验证了框架，确认已知可识别性结果并揭示哪些状态可观测的新见解，为分析动力系统结构特性提供了统一的对称性方法
