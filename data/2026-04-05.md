...
TL;DR: 该研究将小鼠中发现的REM睡眠倾向性分析方法扩展到人类和大鼠，发现在所有三个物种中，NREM睡眠时间积累都会增加REM睡眠倾向性，且REM睡眠倾向性与REM睡眠持续时间正相关。


<details>
  <summary>Details</summary>
Motivation: 哺乳动物睡眠中NREM-REM周期的调控机制尚不清楚，REM睡眠压力被认为是重要因素。先前在小鼠中发现NREM睡眠时间是REM睡眠压力的主要贡献者，本研究旨在验证这一发现在人类和大鼠中是否同样成立。

Method: 将先前在小鼠中开发的REM睡眠倾向性测量方法（定义为在积累额外NREM睡眠之前进入REM睡眠的概率）扩展到分析人类和大鼠的超昼夜NREM-REM周期数据，比较三个物种的周期分布和倾向性特征。

Result: 人类和大鼠睡眠中都包含短NREM-REM序列周期和较长的单NREM-REM周期，尽管周期持续时间分布存在差异。在所有三个物种中，REM睡眠倾向性随NREM睡眠时间增加而上升至峰值，随后随额外NREM睡眠时间而衰减。REM睡眠起始时的倾向性与REM睡眠持续时间在人类和大鼠数据中同样呈正相关。

Conclusion: NREM睡眠时间对REM睡眠倾向性和持续时间的影响在小鼠、人类和大鼠中具有保守性，表明这一机制在哺乳动物睡眠调控中具有普遍性，尽管不同物种的睡眠模式（多相vs单相）存在差异。
...
TL;DR: 该研究开发了数学模型分析肿瘤在免疫反应和治疗干预下的进化动力学，重点关注耐药肿瘤细胞亚群的出现机制和治疗策略设计。


<details>
  <summary>Details</summary>
Motivation: 肿瘤在免疫反应和治疗干预下的进化与生态动力学对长期治疗成功构成重大挑战。尽管治疗初期可能实现短期疾病控制，但耐药肿瘤细胞亚群经常出现，导致疾病复发并形成更具侵袭性和治疗抵抗性的形式。

Method: 开发并分析了描述效应细胞、化疗耐药肿瘤细胞和免疫耐药肿瘤细胞在多种免疫逃逸策略下相互作用的数学模型。模型包含了耐药和敏感肿瘤亚群之间的竞争与合作关系。

Result: 识别了在不同治疗强度下控制肿瘤持续存在、消除和表型优势的阈值条件。这些发现为设计靶向和联合疗法提供了理论基础。

Conclusion: 研究结果为设计靶向和联合疗法提供了理论框架，并为减轻治疗抵抗性提供了策略见解，有助于应对肿瘤进化带来的治疗挑战。
...
<details>
  <summary>Details</summary>
Motivation: 有效建模不规则采样的纵向数据对于理解疾病进展和改善风险预测至关重要，特别是在发现具有异质性轨迹的患者亚组方面

Method: 使用双视图混合模型整合静态基线协变量和纵向生物标志物轨迹，采用神经常微分方程建模时间模式，通过带稀疏诱导对数惩罚的EM算法进行可解释亚组发现

Result: 应用于爱尔兰ANCA相关性血管炎患者队列，揭示了具有异质性血清肌酐轨迹和终末期肾病结局变化的亚组

Conclusion: 该模型能够有效整合多模态数据，发现具有临床意义的患者亚组，为疾病进展建模和风险分层提供了有力工具
...
TL;DR: 比较五种膝关节接触力学替代模型架构在不同输入条件下的性能，发现混合模型在完整输入下表现最佳，但在最小输入条件下最优模型取决于具体任务需求。


<details>
  <summary>Details</summary>
Motivation: 膝关节接触力学的准确替代建模对于重建应力分布和识别风险区域很重要，但在实际输入受限条件下，不同建模范式的相对适用性尚不明确。

Method: 9名男性足球运动员进行90°变向试验，将基于个体特定关节姿态和反作用力的有限元模拟转换为图结构样本，比较五种替代架构（局部扩散、历史上下文增强、分层多尺度建模、显式全局交互、局部-全局混合）在完整、姿态损坏、载荷损坏和最小输入条件下的性能。

Result: 混合模型在完整输入下表现最佳，在姿态和载荷损坏条件下最稳健。在最小输入条件下，没有单一模型在所有指标上占优：历史上下文模型总体和高应力误差较低，混合模型更好地保留高风险区域重建，分层模型在热点定位方面有优势。

Conclusion: 膝关节接触力学替代模型的评估应从理想输入下的准确性比较，转向在现实输入约束下风险相关信息保留的全面评估。虽然局部-全局混合模型显示出最佳的整体稳健性，但在最小输入条件下的最优模型仍取决于具体任务。
...
TL;DR: 该研究使用可解释的EEG特征（标准特征和动态特征）结合多注意力头Transformer分类器，成功区分帕金森病患者的不同状态，发现动态特征能揭示疾病相关的皮层网络组织改变。


<details>
  <summary>Details</summary>
Motivation: 帕金森病会改变皮层神经动力学，但目前缺乏可靠的非侵入性电生理生物标志物。本研究旨在探索可解释的EEG特征是否能有效区分帕金森病的不同神经状态。

Method: 提取全面的可解释EEG特征，分为标准特征（频谱功率、相位同步、时域统计）和动态特征（非周期性活动、跨频率耦合、无标度动力学、神经元雪崩统计、瞬时频率测量）。使用多注意力头Transformer分类器，采用严格的留一受试者交叉验证。进行组间比较以识别与疾病和药物状态相关的电生理差异。

Result: 标准特征在区分药物状态（PDoff vs PDon）方面表现最强，而动态特征在区分患者与健康对照方面表现有竞争力。随机特征消融分析显示动态特征提供互补信息，相关性分析显示两组特征内部冗余度低。组间比较揭示了药物敏感的delta功率和电压方差降低、神经元雪崩统计的调节、PD患者theta相位同步持续增加，以及疾病相关的跨频率相互作用改变。

Conclusion: 传统频谱和同步特征主要反映药物相关的神经调节，而动态描述符揭示了与疾病（也与药物）相关的皮层网络组织更广泛的改变。多变量EEG表征是开发帕金森病非侵入性生物标志物的有前景框架。
...
### [6] [QuantumXCT: Learning Interaction-Induced State Transformation in Cell-Cell Communication via Quantum Entanglement and Generative Modeling](https://arxiv.org/abs/2604.02203)
*Selim Romero,Shreyan Gupta,Robert S. Chapkin,James J. Cai*

Main category: cs.ET

TL;DR: QuantumXCT：一种混合量子-经典生成框架，通过将细胞状态编码到希尔伯特空间并学习酉变换，从单细胞转录组数据推断细胞间通讯，无需依赖配体-受体数据库。


<details>
  <summary>Details</summary>
Motivation: 现有基于配体-受体数据库的细胞间通讯推断方法主要捕获共表达而非信号传导对细胞状态的系统级影响，存在根本性局限。

Method: 将转录组谱编码到高维希尔伯特空间，训练参数化量子电路学习将基线非相互作用细胞状态映射到相互作用状态的酉变换，无需先验生物学假设。

Result: 在合成数据和卵巢癌-成纤维细胞共培养系统的单细胞RNA-seq数据中准确恢复复杂调控依赖关系（包括反馈结构），识别PDGFB-PDGFRB-STAT3轴等主要通讯枢纽，量子电路可解释。

Conclusion: QuantumXCT将细胞间通讯推断从静态相互作用查找转变为数据驱动的状态变换学习，为复杂生物系统中通讯程序的新发现建立了新范式，展示了量子机器学习在单细胞生物学中的潜力。
...
TL;DR: annbatch是一个用于anndata的mini-batch加载器，支持直接在磁盘数据集上进行核心外训练，显著提升生物数据加载效率


<details>
  <summary>Details</summary>
Motivation: 生物数据集规模已超过系统内存容量，数据访问成为机器学习训练的主要瓶颈。现有生物数据格式需要支持异构元数据、稀疏和密集检测以及下游分析，但缺乏高效的数据加载机制

Method: 开发annbatch作为anndata的原生mini-batch加载器，支持直接在磁盘数据集上进行核心外训练，保持与scverse生态系统的完全兼容性
...
### [8] [Multipath Channel Metrics and Detection in Vascular Molecular Communication: A Wireless-Inspired Perspective](https://arxiv.org/abs/2604.01362)
...
<details>
  <summary>Details</summary>
Motivation: 早期分子通信研究主要借鉴无线电磁通信系统的建模方法，但在复杂血管网络环境中，现有模型缺乏解析可处理性，阻碍了对大规模血管网络的严格通信理论分析。

Method: 利用MIGHT（血流传输的混合逆高斯）模型，推导泊松信道噪声模型，建立多径无线通信与血管网络中分子通信的结构类比，定义RMS延迟扩展、平均超额延迟和相干带宽等经典指标，推导信道频率响应和功率延迟剖面的闭式表达式。

Result: 建立了血管网络中分子通信与多径无线通信的类比关系，提出了血管网络适应的相干决策反馈检测器，并展示了如何利用多径指标指导关键系统参数的选择，在不同血管网络中评估了检测器在符号间干扰下的性能。

Conclusion: 这项工作为大规模血管网络中系统性的、受多径无线通信启发的分子通信系统设计打开了大门，首次实现了对复杂大规模血管网络的通信理论分析。
